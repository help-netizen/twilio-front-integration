...
describe('keycloakAuth — dev mode (FEATURE_AUTH_ENABLED=false)', () => {
    ...
    // ONBOARD-FIX-001 (SEC): the dev bypass must NEVER hand out the seed company
    // in production — fail closed instead of leaking Boston Masters to everyone.
    test('authenticate fails closed (500 AUTH_MISCONFIGURED) if dev bypass would run in production', () => {
...
// ─── requireCompanyAccess — tenant isolation (ONBOARD-FIX-001 SEC) ────────────

describe('requireCompanyAccess — tenant scope is membership-only', () => {
    ...
    beforeAll(() => {
        ...
        requireCompanyAccess = require('../backend/src/middleware/keycloakAuth').requireCompanyAccess;
    });
    ...
    const baseReq = (over) => ({
        method: 'GET', originalUrl: '/api/pulse', ip: '127.0.0.1',
        user: { crmUser: { id: 'u1' }, email: 'u@x.com' },
        ...over,
    });

    // THE leak regression: no active membership → authz.company is null. The old
    // fallback to req.user.company_id (backfilled to the seed company) must be gone.
    test('DENIES (403 TENANT_CONTEXT_REQUIRED) when no membership, even if shadow company_id = seed', () => {
        const req = baseReq({
            user: { crmUser: { id: 'u1' }, email: 'u@x.com', company_id: SEED }, // stale shadow
            authz: { company: null, scope: null, platform_role: 'none' },
        });
        const next = jest.fn();

        requireCompanyAccess(req, mockRes, next);

        expect(next).not.toHaveBeenCalled();
        expect(mockRes.status).toHaveBeenCalledWith(403);
        expect(mockRes.json).toHaveBeenCalledWith(expect.objectContaining({ code: 'TENANT_CONTEXT_REQUIRED' }));
        expect(req.companyFilter).toBeUndefined(); // never scoped to the seed company
    });

    test('ALLOWS and scopes to the membership company when authz.company is set', () => {
        const req = baseReq({
            user: { crmUser: { id: 'u1' }, email: 'u@x.com', company_id: SEED }, // shadow ignored
            authz: { company: { id: 'company-A' }, scope: 'tenant', platform_role: 'none' },
        });
        const next = jest.fn();
        ...
        expect(next).toHaveBeenCalledTimes(1);
        expect(req.companyFilter).toEqual({ company_id: 'company-A' });
        expect(mockRes.status).not.toHaveBeenCalled();
    });

    test('DENIES platform-only (super_admin) on tenant routes (403 PLATFORM_SCOPE_ONLY)', () => {
        const req = baseReq({
            authz: { company: null, scope: 'platform', platform_role: 'super_admin' },
        });
        const next = jest.fn();
        ...
        expect(next).not.toHaveBeenCalled();
        expect(mockRes.status).toHaveBeenCalledWith(403);
        expect(mockRes.json).toHaveBeenCalledWith(expect.objectContaining({ code: 'PLATFORM_SCOPE_ONLY' }));
    });
});
