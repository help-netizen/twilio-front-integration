...
const SQL_ALLOWLIST = new Map([
    ...
    ['R-write-scope:backend/src/routes/leads.js:326', 'Pre-existing unscoped route write; remediation deferred to the post-audit triage.'],
    ['R-write-scope:backend/src/routes/leads.js:364', 'Pre-existing unscoped route write; remediation deferred to the post-audit triage.'],
    ['R-write-scope:backend/src/routes/leads.js:430', 'Pre-existing unscoped route write; remediation deferred to the post-audit triage.'],
    ['R-write-scope:backend/src/routes/leads.js:493', 'Pre-existing unscoped route write; remediation deferred to the post-audit triage.'],
    ...
    ['R-no-request-context:backend/src/services/inboxWorker.js:806', 'Pre-existing worker write lacks explicit companyId; remediation deferred.'],
...
const ROUTE_PERMISSION_EXCEPTIONS = new Map([
    ...
    ['backend/src/routes/notification-policies.js:router:GET:/notification-policies', 'Authenticated tenant members read company policy, only their own role delivery unless tenant.company.manage, and only their own preferences.'],
    ['backend/src/routes/notification-policies.js:router:PATCH:/notification-preferences/:eventType', 'Authenticated tenant members may update only their own company/user preference tuple; identity is request-derived.'],
