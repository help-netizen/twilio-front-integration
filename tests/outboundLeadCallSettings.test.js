...
describe('TC-OLC-013: calling-window settings (OLC-WINDOW-001)', () => {
    ...
    it('saveSettings persists a custom window; NON-custom modes null the custom times', async () => {
        db.query.mockResolvedValue({ rows: [{
            enabled_sources: ['X'], calling_window_mode: 'custom', custom_start_time: '10:00', custom_end_time: '22:00',
            calling_window_work_days: [1, 2, 3, 4, 5],
        }] });
        await svc.saveSettings('co-1', {
            enabled_sources: ['X'], calling_window_mode: 'custom', custom_start_time: '10:00', custom_end_time: '22:00',
            calling_window_work_days: [1, 2, 3, 4, 5],
        });
        expect(db.query.mock.calls[0][1]).toEqual([
            'co-1', JSON.stringify(['X']), 'custom', '10:00', '22:00', JSON.stringify([1, 2, 3, 4, 5]),
        ]);
        ...
        expect(db.query.mock.calls[0][1]).toEqual(['co-1', JSON.stringify([]), 'always', null, null, null]);
    });

    it('saveSettings downgrades an invalid custom window to inheritance (never persists garbage)', async () => {
        db.query.mockResolvedValue({ rows: [{ enabled_sources: [], calling_window_mode: null }] });
        await svc.saveSettings('co-1', { enabled_sources: [], calling_window_mode: 'custom', custom_start_time: '22:00', custom_end_time: '09:00' });
        expect(db.query.mock.calls[0][1]).toEqual(['co-1', JSON.stringify([]), null, null, null, null]);
    });
});
