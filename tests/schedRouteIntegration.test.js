...
describe('createFromSlot(job) — FR-001', () => {
    it('creates a manual job and enqueues geocode + recalc when address has no coords', async () => {
        ...
        expect(jobsService.createManualJob).toHaveBeenCalledWith(
            'co',
            expect.objectContaining({ address: '123 Main', assignee_id: 'u1' }),
            null
        );
    ...
    it('accepts a legacy technician input but stores the canonical UUID', async () => {
        jobsService.createManualJob.mockResolvedValue({ id: 8 });
        await scheduleService.createFromSlot('co', 'job', {
            title: 'x',
            assigned_techs: [{ id: 'zb-old', name: 'Historical name' }],
        });
        expect(jobsService.createManualJob).toHaveBeenCalledWith(
            'co',
