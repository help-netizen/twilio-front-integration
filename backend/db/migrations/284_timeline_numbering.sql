-- TIMELINE-NUMBERING: global durable codes for timelines.
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_timelines_public_code
    ON timelines(public_code);

-- This is the same four-round Feistel/cycle-walk construction used by
-- job_public_code and lead_public_code. Timelines deliberately reuse the
-- existing jobs GUC key; the route prefix provides a separate namespace.
CREATE OR REPLACE FUNCTION timeline_public_code(p_id BIGINT)
RETURNS TEXT
LANGUAGE plpgsql
STABLE
STRICT
...
BEGIN
    IF p_id < 0 OR p_id >= v_domain_size THEN
        RAISE EXCEPTION
            'timeline id % is outside the 5-character public-code domain [0, %)',
            p_id,
            v_domain_size
            USING ERRCODE = '22003';
    END IF;
...
COMMENT ON FUNCTION timeline_public_code(BIGINT) IS
    'Feistel/cycle-walk id->5-char base62 timeline code. Reuses GUC app.job_code_feistel_key.';

CREATE OR REPLACE FUNCTION timelines_assign_public_code()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path FROM CURRENT
AS $function$
BEGIN
    IF NEW.public_code IS NULL THEN
        NEW.public_code := timeline_public_code(NEW.id);
    END IF;
    RETURN NEW;
END;
$function$;
...
CREATE TRIGGER trg_timelines_assign_public_code
    BEFORE INSERT ON timelines
    FOR EACH ROW
    EXECUTE FUNCTION timelines_assign_public_code();

UPDATE timelines
SET public_code = timeline_public_code(id)
