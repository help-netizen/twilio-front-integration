...
async function findByCodeScoped(companyId, code, client = null) {
    const query = queryFor(client);
    const { rows } = await query(
        `SELECT ${COLUMNS} FROM estimate_item_presets
         WHERE company_id = $1 AND archived_at IS NULL AND lower(btrim(code)) = lower(btrim($2))
         LIMIT 1`,
        [companyId, code],
    );
    return rows[0] || null;
...
async function listForManage(companyId, { search = '', category_id = null, uncategorized = false, includeArchived = false, limit = 50, offset = 0 } = {}, client = null) {
    const query = queryFor(client);
    const params = [companyId];
    let where = `p.company_id = $1`;
    if (!includeArchived) where += ` AND p.archived_at IS NULL`;
    if (uncategorized) where += ' AND p.category_id IS NULL';
    else if (category_id != null) { params.push(category_id); where += ` AND p.category_id = $${params.length}`; }
    if (search && search.trim()) {
        params.push(`%${search.trim()}%`);
    ...
    const { rows } = await query(
        `SELECT ${COLUMNS.split(',').map(c => 'p.' + c.trim()).join(', ')}, c.name AS category_name
         ...
         LEFT JOIN price_book_categories c ON c.id = p.category_id AND c.company_id = p.company_id
         WHERE ${where}
