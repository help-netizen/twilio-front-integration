...
const CAT_COLS = `id, company_id, name, description, sort_order, archived_at, created_by, created_at, updated_at`;
...
async function insertCategory(companyId, { name, description = null, sort_order = 0, createdBy = null }) {
    const { rows } = await db.query(
        `INSERT INTO price_book_categories (company_id, name, description, sort_order, created_by)
         VALUES ($1,$2,$3,$4,$5) RETURNING ${CAT_COLS}`,
        [companyId, name, description, sort_order, createdBy]);
    return rows[0];
}
async function updateCategory(companyId, id, payload) {
    const fields = [], params = [];
    for (const k of ['name', 'description', 'sort_order']) {
...
async function archiveCategory(companyId, id) {
    const { rows } = await db.query(
        `UPDATE price_book_categories SET archived_at = NOW(), updated_at = NOW()
         WHERE id = $1 AND company_id = $2 AND archived_at IS NULL RETURNING ${CAT_COLS}`, [id, companyId]);
    return rows[0] || null;
}

// ── Groups ───────────────────────────────────────────────────────────────────
// List with computed item_count + total over ACTIVE member items.
async function listGroups(companyId, { includeArchived = false, search = '' } = {}) {
    ...
    const { rows } = await db.query(
        `SELECT ${GRP_COLS.split(',').map(c => 'g.' + c.trim()).join(', ')},
         ...
         LEFT JOIN price_book_categories c ON c.id = g.category_id
         LEFT JOIN (
             SELECT gi.group_id,
                    count(*) AS item_count,
                    sum(gi.quantity * i.default_unit_price) AS total
             FROM price_book_group_items gi
             JOIN estimate_item_presets i ON i.id = gi.item_id AND i.archived_at IS NULL
...
async function getGroupItems(companyId, groupId) {
    const { rows } = await db.query(
        `SELECT gi.id AS link_id, gi.item_id, gi.quantity, gi.sort_order,
         ...
         FROM price_book_group_items gi
         JOIN estimate_item_presets i ON i.id = gi.item_id
...
async function findCategoryByName(companyId, name) {
    const { rows } = await db.query(
        `SELECT ${CAT_COLS} FROM price_book_categories
...
async function exportRows(companyId) {
    const { rows } = await db.query(
        `SELECT i.name, i.description, i.code, i.unit, i.default_unit_price, i.default_taxable,
         ...
         LEFT JOIN price_book_categories c ON c.id = i.category_id AND c.archived_at IS NULL
         LEFT JOIN price_book_group_items gi ON gi.item_id = i.id
         LEFT JOIN price_book_groups g ON g.id = gi.group_id AND g.archived_at IS NULL
