/**
 * React-PDF document for estimates. Mirrors TemplateLivePreview (HTML) so the
 * generated PDF reflects the saved descriptor exactly: layout_preset typography,
 * font_scale, theme colors, section widths, glue_with_next, ACH inline,
 * text alignment, accent bars, etc.
 *
 * Built with React.createElement (no JSX) to avoid adding Babel to the Node
 * backend. Reads brand/theme/sections from the descriptor; estimate fields are
 * pulled from the live estimate row passed by `estimatesService.generatePdf`.
 */
...
const PRESETS = {
    light: {
        ...
        estimateLabelSize: 8,
        estimateLabelLetterSpacing: 1.6,
        estimateNumberSize: 22,
        estimateNumberWeight: 'bold',
        estimateMetaSize: 10,
        brandNameSize: 16,
        brandNameWeight: 'semibold',
        brandSubSize: 9.5,
        logoMaxHeight: 64,
        itemNameSize: 11,
        ...
        itemDescSize: 9.5,
        itemAmountSize: 11,
        itemAmountWeight: 'semibold',
        rowDividerWidth: 0.5,
        totalsLabelSize: 10,
        totalsValueSize: 10,
        totalLabelSize: 12,
        totalLabelWeight: 'semibold',
        totalValueSize: 16,
    ...
    bold: {
        ...
        estimateLabelSize: 9,
        estimateLabelLetterSpacing: 1.8,
        estimateNumberSize: 32,
        estimateNumberWeight: 'extrabold',
        estimateMetaSize: 11,
        brandNameSize: 22,
        brandNameWeight: 'bold',
        brandSubSize: 11,
        logoMaxHeight: 84,
        itemNameSize: 13,
        itemNameWeight: 'bold',
    ...
    minimal: {
        ...
        estimateLabelSize: 8,
        estimateLabelLetterSpacing: 1,
        estimateNumberSize: 14,
        estimateNumberWeight: 'medium',
        estimateMetaSize: 9,
        brandNameSize: 13,
        brandNameWeight: 'medium',
        brandSubSize: 9,
        logoMaxHeight: 52,
        itemNameSize: 10,
        itemNameWeight: 'medium',
        itemDescSize: 9,
        itemAmountSize: 10,
        itemAmountWeight: 'normal',
        rowDividerWidth: 0.4,
        totalsLabelSize: 9,
        totalsValueSize: 9,
        totalLabelSize: 10,
        totalLabelWeight: 'medium',
        totalValueSize: 12,
        totalValueWeight: 'medium',
...
function widthPct(s) {
    return WIDTH_PCT[s.width || 'full'] || '100%';
}
...
function renderSection(s, descriptor, estimate, tokens) {
    ...
    if (s.key === 'header') {
        ...
        return e(P.View, { style: { ...padding, flexDirection: 'row', alignItems: 'flex-start' } },
            ...
            !hasMeta && e(P.View, { style: { textAlign: 'right' } },
    ...
    if (s.key === 'client_addresses') {
        const billing = estimate.billing_address || estimate.service_address || '';
        const service = estimate.service_address || estimate.billing_address || '';
        const col = (label, addr) => e(P.View, { style: { flexBasis: '50%', flexGrow: 1, paddingRight: 16 } },
            e(SectionHeading, { label, tokens, accent, muted }),
            e(P.Text, { style: { fontSize: 11, fontFamily: 'Helvetica-Bold' } }, estimate.contact_name || 'Customer'),
            e(P.Text, { style: { fontSize: 10, color: muted, marginTop: 1 } }, estimate.contact_email || ''),
            e(P.Text, { style: { fontSize: 10, color: muted } }, estimate.contact_phone || ''),
            e(P.Text, { style: { fontSize: 10, marginTop: 4 } }, addr),
        );
        return e(P.View, { style: { ...padding, flexDirection: 'row' } },
            col('Prepared for', billing),
            ...
            e(P.View, { style: { width: 220 } },
                ...
                e(P.View, {
                    style: {
                        ...
                        borderTopWidth: tokens.totalDividerWidth,
...
// Cached primitives (set on first call).
let P = null;
function buildEstimatePdfElement({ estimate, descriptor }, primitives) {
    P = primitives;
    return e(EstimatePdfDocument, { estimate, descriptor });
}

function EstimatePdfDocument({ estimate, descriptor }) {
            ...
            e(P.View, { style: { flexDirection: 'row', flexWrap: 'wrap' } },
                ...groups.map((group, gIdx) => {
                    ...
                    if (group.length === 1) {
                        ...
                        return e(P.View, {
                            key: 'g' + gIdx,
                            style: { width: widthPct, textAlign: align },
                        }, renderSection(s, descriptor, estimate, tokens));
                    }
                    return e(P.View, {
                        key: 'g' + gIdx,
                        style: { width: widthPct, flexDirection: 'row', alignItems: 'flex-start' },
                    }, ...group.map((s, i) => e(P.View, {
                        key: i,
                        style: { textAlign: alignFor(s) },
                    }, renderSection(s, descriptor, estimate, tokens))));
                })
            )
        )
...
module.exports = { buildEstimatePdfElement };
