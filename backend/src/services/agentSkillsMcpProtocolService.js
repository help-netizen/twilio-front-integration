...
/**
 * agentSkillsMcpProtocolService — JSON-RPC protocol for the service-CRM (`svc.*`)
 * MCP surface. AGENT-SKILLS-001, AR-3 / spec §8 / architecture §4.
 *
 * MIRRORS `crmMcpProtocolService.js` byte-for-byte in shape (same
 * `initialize`/`ping`/`tools/list`/`tools/call` handling, same `toProtocolTool`
 * annotations, same `crmMcpResponse.mapError` error-code mapping) but points at
 * the `svc.*` registry + executor. The ONLY behavioral differences:
 *   - `serverInfo.name = 'albusto-service-crm-mcp'` (distinct from the sales
 *     server's `blanc-sales-crm-mcp`);
 *   - `toProtocolTool` also surfaces the projected `requiredLevel` in
 *     annotations so a caller sees the verification bar the skill layer enforces.
 *
 * `crmMcpResponse` is REUSED UNCHANGED, so the sanitized-error contract (drops
 * token/secret/password/oauth/sql/stack, truncates strings) is inherited (G6).
 * The sales protocol service is UNTOUCHED — additive only.
 */
