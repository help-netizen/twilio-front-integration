...
# ⚠️ CRITICAL: Zenbooker webhook_id — это ID подписки, НЕ ID доставки

- `webhook_id` из payload — это идентификатор **подписки на тип события** (subscription ID)
- Он **одинаковый** для всех доставок одного типа (напр. все `job.rescheduled` одной работы имеют один `webhook_id`)
- **НИКОГДА** не использовать `webhook_id` для дедупликации или как уникальный ID события
- Для `event_key` использовать: `zenbooker:${event}:${dataId}:${reqId}` — где `reqId` генерируется нашим сервером при получении HTTP-запроса
