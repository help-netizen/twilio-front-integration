 ...
 *
 * Layout: the app's full-width page shell (warm bg, header spanning the content
 * area) with a comfortable readable column for the template list — Blanc tokens
 * and ui/ components throughout.
 */
...
export default function QuickMessagesPage() {
    ...
    return (
        <div className="blanc-page-wrapper" style={{ color: 'var(--blanc-ink-1)' }}>
            ...
            <div className="flex items-center justify-between gap-4" style={{ padding: '0 4px' }}>
                <div>
                    <h1
                    ...
                    <p className="text-sm mt-1" style={{ color: 'var(--blanc-ink-3)' }}>
                        Reusable message templates. Drag to reorder.
                    </p>
                </div>
                {qm.saving && (
            ...
            {/* Scrollable content — comfortable readable width, room on the right for the VariablePicker */}
            <div className="flex-1 min-h-0 overflow-y-auto" style={{ padding: '4px 4px 24px' }}>
                <div style={{ maxWidth: 860, paddingRight: 140 }}>
                    {qm.showAddForm ? (
                        <div
                            className="mb-4 flex flex-col gap-3 rounded-2xl p-4"
