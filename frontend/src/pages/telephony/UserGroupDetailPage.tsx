...
import { Users, Settings, GitBranch, Play, Phone, Calendar, Clock, Plus, Trash2, AlertTriangle, CheckCircle } from 'lucide-react';
...
const card: React.CSSProperties = { background: 'var(--blanc-panel-surface)', border: '1px solid var(--blanc-line)', borderRadius: 12, padding: 20 };
...
export default function UserGroupDetailPage() {
    ...
    if (loading) return (
        <SettingsPageShell backTo="/settings/telephony/user-groups" backLabel="User Groups" title="…">
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--blanc-ink-3)' }}>Loading...</div>
        </SettingsPageShell>
    );
    if (!group) return (
        <SettingsPageShell backTo="/settings/telephony/user-groups" backLabel="User Groups" title="Group not found">
            <></>
        </SettingsPageShell>
    );
    ...
    return (
        <SettingsPageShell
            backTo="/settings/telephony/user-groups"
            backLabel="User Groups"
            title={group.name}
            description={[group.desc, 'Ring strategy: Simultaneous'].filter(Boolean).join(' · ')}
            actions={editingName ? (
                <>
                    <input value={nameDraft} onChange={e => setNameDraft(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') saveName(); if (e.key === 'Escape') setEditingName(false); }} style={{ fontSize: 14, fontWeight: 600, border: '1px solid var(--blanc-line-strong)', borderRadius: 8, padding: '6px 10px', background: 'var(--blanc-panel-surface)', color: 'var(--blanc-ink-1)' }} autoFocus />
                    <Button size="sm" onClick={saveName} disabled={savingName || !nameDraft.trim()}>{savingName ? 'Saving...' : 'Save'}</Button>
                </>
            ) : (
                <>
        ...
        >
            {!reachable && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', border: '1px solid #fecaca', borderRadius: 8, background: '#fef2f2', color: '#991b1b', fontSize: 13, fontWeight: 600 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div style={card}>
                    ...
                    {group.members.map(m => {
                        ...
                        return (
                            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid var(--blanc-line)' }}>
                    ...
                    {availableAgents.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
                            {availableAgents.map(agent => (
                                <button key={agent.id} disabled={savingInline} onClick={() => addMember(agent)} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 9px', fontSize: 12, fontWeight: 600, background: 'rgba(25,25,25,0.04)', color: 'var(--blanc-ink-1)', border: '1px solid var(--blanc-line)', borderRadius: 6, cursor: savingInline ? 'default' : 'pointer' }}>
                ...
                <div style={card}>
                    <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
                        <Phone size={15} />Assigned Numbers ({group.numbers.length})
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--blanc-ink-2)', marginBottom: 10 }}>Inbound calls on these numbers route through this group's flow</div>
                    {group.numbers.map(n => (
                        <div key={n.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: '1px solid var(--blanc-line)' }}>
                            <Phone size={13} style={{ color: 'var(--blanc-accent)' }} />
                            <span style={{ fontSize: 13, fontWeight: 500, flex: 1 }}>{n.number}</span>
                            <span style={{ fontSize: 11, color: 'var(--blanc-ink-2)' }}>{n.friendly_name}</span>
                    ...
                    {availableNumbers.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
                            {availableNumbers.map(number => (
                                <button key={number.id} disabled={savingInline} onClick={() => addNumber(number)} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 9px', fontSize: 12, fontWeight: 600, background: 'rgba(25,25,25,0.03)', color: 'var(--blanc-ink-1)', border: '1px solid var(--blanc-line)', borderRadius: 6, cursor: savingInline ? 'default' : 'pointer' }}>
                ...
                <div style={card}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: scheduleStatus.isOpen ? '#d1fae5' : 'rgba(25,25,25,0.05)', color: scheduleStatus.isOpen ? '#047857' : 'var(--blanc-ink-2)' }}>{scheduleStatus.shortLabel}</span>
                            <span style={{ fontSize: 11, color: 'var(--blanc-ink-2)' }}>{group.schedule.timezone}</span>
                        </div>
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--blanc-ink-2)', marginBottom: 8 }}>{scheduleStatus.label}</div>
                    {group.schedule.hours.map(h => (
                        <div key={h.day} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid var(--blanc-line)', fontSize: 12 }}>
                            <span style={{ fontWeight: 500, minWidth: 40 }}>{h.day}</span>
                            <span style={{ color: h.open === 'Closed' ? '#ef4444' : 'var(--blanc-ink-1)', display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <div style={card}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <div style={{ fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
                            <GitBranch size={15} />Call Flow
                        </div>
                        <Button size="sm" onClick={() => navigate(`/settings/telephony/user-groups/${groupId}/flow`)}>
                            <Play />Open Builder
                        </Button>
                    </div>
                        ...
                        <>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                                {flowStates.map(s => {
                                    ...
                                    return (
                                        <div key={s.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', background: 'rgba(25,25,25,0.03)', borderRadius: 6, fontSize: 12, border: '1px solid var(--blanc-line)' }}>
                                            <span>{meta.icon}</span>
                    ...
                    ) : <div style={{ fontSize: 12, color: 'var(--blanc-ink-3)' }}>No flow configured</div>}
                </div>
            </div>
        </SettingsPageShell>
    );
