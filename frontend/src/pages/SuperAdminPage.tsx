...
export default function SuperAdminPage() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try { const [sR, pR] = await Promise.all([fetch(`${API_BASE}/admin/sessions`, { headers }), fetch(`${API_BASE}/admin/sessions/auth-policy`, { headers })]); if (sR.ok) { const d = await sR.json(); setSessions(d.sessions || []); } else toast.error('Failed to load sessions'); if (pR.ok) { const d = await pR.json(); setPolicy(d.policy || null); } else toast.error('Failed to load auth policy'); }
        catch (e: any) { toast.error('Connection error', { description: e.message }); } finally { setLoading(false); }
    };

    useEffect(() => { fetchData(); }, []);

    const revokeSession = async (sessionId: string) => { setRevoking(sessionId); try { const r = await fetch(`${API_BASE}/admin/sessions/${sessionId}`, { method: 'DELETE', headers }); if (r.ok) { setSessions(p => p.filter(s => s.id !== sessionId)); toast.success('Session revoked'); } else toast.error('Failed to revoke session'); } catch { toast.error('Failed to revoke session'); } setRevoking(null); };
    const revokeAllSessions = async () => { setRevoking('all'); try { const userIds = [...new Set(sessions.map(s => s.userId))]; await Promise.all(userIds.map(uid => fetch(`${API_BASE}/admin/sessions/user/${uid}`, { method: 'DELETE', headers }))); setSessions([]); toast.success('All sessions revoked'); } catch { toast.error('Failed to revoke sessions'); } setRevoking(null); };

    return (
        <div className="max-w-4xl mx-auto p-6 space-y-8">
            <div><div className="flex items-center gap-2 mb-1"><Shield className="size-5 text-muted-foreground" /><h2 className="text-xl font-semibold">Super Admin</h2></div><p className="text-sm text-muted-foreground">Session management, authentication policy, and admin tools.</p></div>
            <section className="space-y-3"><h3 className="font-medium">Quick Links</h3><div className="grid grid-cols-1 sm:grid-cols-3 gap-3">{quickLinks.map(link => <a key={link.title} href={link.href} target="_blank" rel="noopener noreferrer" className="group block"><Card className="h-full transition-colors hover:border-primary/40"><CardHeader className="pb-2"><div className="flex items-center justify-between"><link.icon className="size-4 text-muted-foreground" /><ExternalLink className="size-3 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" /></div><CardTitle className="text-sm">{link.title}</CardTitle></CardHeader><CardContent><p className="text-xs text-muted-foreground">{link.description}</p></CardContent></Card></a>)}</div></section>
            <Separator />
            <section className="space-y-3"><h3 className="font-medium">Auth Policy</h3>{loading ? <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">{[...Array(6)].map((_, i) => <Skeleton key={i} className="h-20 w-full" />)}</div> : policy ? <div className="grid grid-cols-2 sm:grid-cols-3 gap-3"><PolicyCard icon={Clock} label="Access Token" value={fmt(policy.session.accessTokenLifespan)} /><PolicyCard icon={Clock} label="Session Idle" value={fmt(policy.session.ssoSessionIdleTimeout)} /><PolicyCard icon={Clock} label="Session Max" value={fmt(policy.session.ssoSessionMaxLifespan)} /><PolicyCard icon={Lock} label="Password" value={policy.password.raw || 'Default'} /><PolicyCard icon={Fingerprint} label="MFA" value={policy.mfa.otpPolicyType || 'N/A'} /><PolicyCard icon={Shield} label="Brute Force" value={policy.bruteForce.enabled ? `On (${policy.bruteForce.failureFactor} attempts)` : 'Off'} variant={policy.bruteForce.enabled ? 'default' : 'destructive'} /></div> : <p className="text-sm text-muted-foreground">Failed to load policy.</p>}</section>
            <Separator />
