...
import { ChevronRight, Clock, Layers, Loader2, Plus, Search, X } from 'lucide-react';
import { useIsMobile } from '../../hooks/useIsMobile';
import { useKeyboardInset } from '../shared/NoteComposerOverlay';
...
export function ItemPresetSearchCombobox({ disabled, onPickPreset, onCreateNew, onPickGroup }: Props) {
    ...
    useEffect(() => {
        if (!open || isMobile) return; // mobile uses a full-screen surface, not a click-outside dropdown
    ...
    // The list content — identical on desktop (in the dropdown) and mobile (full-screen).
    const listRows = (
        <>
            {!trimmed && (
    ...
    // Mobile: full-screen search (type C) — input pinned top, list scrolls below the keyboard.
    if (isMobile) {
        return (
            <>
                <button
                    ...
                    className="flex h-9 w-full items-center gap-2 rounded-[10px] bg-[var(--blanc-accent-soft)] px-3 text-left text-sm disabled:opacity-50"
                ...
                {open && createPortal(
                    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, pointerEvents: 'auto', background: 'var(--blanc-surface-strong)', display: 'flex', flexDirection: 'column' }} role="dialog" aria-modal="true">
                        ...
                        <div className="flex-1 overflow-y-auto" style={{ paddingBottom: keyboardInset }}>
                            {listRows}
                        </div>
                    </div>,
                    document.body,
