...
export function useConvertToJob(lead: Lead, open: boolean, onSuccess: (lead: Lead) => void, onOpenChange: (open: boolean) => void) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.success('Job created', { description: `Job #${result.data?.job_seq ?? jobId}`, duration: 10000, action: jobId ? { label: 'Open Job', onClick: () => navigate(`/jobs/by-id/${jobId}`) } : undefined });
