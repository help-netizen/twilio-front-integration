...
export const MobileMoreSheet: React.FC<{ open: boolean; onClose: () => void; logout: () => void; activeTab: string }> = ({ open, onClose, logout, activeTab }) => {
    ...
    const groups = getVisibleSettingsGroups({ permissions, platformRole, tenantRole: membership?.role_key, companyFlags: { app_studio_enabled: company?.app_studio_enabled } });
