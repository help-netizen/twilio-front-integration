    ...
    return (
        <div className="border-t p-4" style={{ borderTopColor: 'var(--blanc-line)', background: 'var(--blanc-surface-strong)' }}>
            {hasSecondary && (
                <div className="mb-2 relative">
                    <div className="flex items-center gap-2 text-xs">
                        ...
                        <button type="button" onClick={() => setToDropdownOpen(!toDropdownOpen)} className="flex items-center gap-1.5 px-2 py-1 rounded-md border hover:bg-muted/60 transition-colors text-sm" style={{ borderColor: 'var(--blanc-line)', background: 'var(--blanc-surface-muted)', color: 'var(--blanc-ink-1)' }}>
            ...
            <div className="relative mb-3">
                ...
                <div className="absolute top-2 right-3 text-xs" style={{ color: 'var(--blanc-ink-3)' }}>{message.length} characters</div>
            </div>
            <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                    <div className="relative">
                        <button onClick={() => setIsPresetsOpen(!isPresetsOpen)} className="flex items-center gap-1.5 px-3 py-1.5 text-sm hover:bg-muted/60 rounded-lg transition-colors" style={{ color: 'var(--blanc-ink-2)' }}><ChevronDown className={`w-4 h-4 transition-transform ${isPresetsOpen ? 'rotate-180' : ''}`} /><span>Quick Messages</span></button>
                        {isPresetsOpen && (<><div className="fixed inset-0 z-10" onClick={() => setIsPresetsOpen(false)} /><div className="absolute left-0 bottom-full mb-1 w-64 rounded-xl shadow-lg z-20 py-1" style={{ background: 'var(--blanc-surface-strong)', border: '1px solid var(--blanc-line)' }}>{quickMessages.map(qm => <button key={qm.id} onClick={() => handlePresetSelect(qm.content)} className="w-full text-left px-3 py-2 hover:bg-muted/50 transition-colors"><div className="text-sm font-medium" style={{ color: 'var(--blanc-ink-1)' }}>{qm.title}</div><div className="text-xs line-clamp-1" style={{ color: 'var(--blanc-ink-3)' }}>{qm.content}</div></button>)}<div className="border-t my-1" style={{ borderColor: 'var(--blanc-line)' }} /><button onClick={() => { setIsPresetsOpen(false); navigate('/settings/quick-messages'); }} className="w-full text-left px-3 py-2 hover:bg-muted/50 transition-colors"><div className="text-sm font-medium" style={{ color: 'var(--blanc-info)' }}>+ Add New</div></button></div></>)}
                    </div>
