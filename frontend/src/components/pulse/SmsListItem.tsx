...
const handleDownload = (media: SmsMediaItem, signedUrl: string) => {
        ...
        return (
            <div className="relative group">
                <img
                    ...
                    onClick={() => signedUrl && handleDownload(media, signedUrl)}
                />
                <button
                    onClick={(e) => { e.stopPropagation(); if (signedUrl) handleDownload(media, signedUrl); }}
            ...
            >
                ...
                {hasMedia && (
                    <div className={`${hasMessage ? 'pb-0' : ''} p-1.5 space-y-1.5`}>
                        {sms.media!.map((media) => (
                            <div key={media.id}>
                                <SmsMediaAttachment media={media} isOutgoing={isOutgoing} />
                            </div>
                        ))}
                    </div>
                )}
