...
>(({ className, children, size = "default", variant = "dialog", ...props }, ref) => (
    <DialogPortal>
        ...
        <DialogPrimitive.Content
            ref={ref}
            className={cn(
                // Mobile: bottom-sheet — full-width, pinned to bottom, slides up (both variants)
                ...
                variant === "panel"
                    // Desktop: right-side slide-in layer (matches the job-detail card)
                    ? "md:left-auto md:top-[72px] md:bottom-5 md:right-5 md:h-auto md:translate-x-0 md:translate-y-0 md:w-[600px] md:max-w-[calc(100vw-40px)] md:rounded-[22px] md:content-start md:overflow-y-auto md:data-[state=open]:animate-[blancSlideInRight_0.24s_ease-out] md:data-[state=closed]:animate-out md:data-[state=closed]:fade-out-0 md:data-[state=closed]:slide-out-to-right-8"
                    // Desktop: centered modal; width by `size`
                    : cn(
