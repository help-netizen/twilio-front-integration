...
type Scenario = 'connected' | 'fresh' | 'disabled';
...
function body() {
    ...
    const me = scenario === 'connected'
        ? { connected: true, base: 'chatgpt', mode: 'mcp', writes_enabled: writes, sends_enabled: sends }
        : null;
    const roster = enabled ? [
        { owner_user_id: 'me', owner_name: 'Rustam G.', base: 'chatgpt', connection_status: 'connected', presence: 'active', is_me: true },
        { owner_user_id: 'u2', owner_name: 'Maria K.', base: 'chatgpt', connection_status: 'connected', presence: 'active', is_me: false },
...
window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (url.includes('/api/avatars/me/writes')) { writes = JSON.parse(String(init?.body || '{}')).enabled; return ok(JSON.stringify({ connected: true, base: 'chatgpt', mode: 'mcp', writes_enabled: writes, sends_enabled: sends })); }
    if (url.includes('/api/avatars/me/sends')) { sends = JSON.parse(String(init?.body || '{}')).enabled; return ok(JSON.stringify({ connected: true, base: 'chatgpt', mode: 'mcp', writes_enabled: writes, sends_enabled: sends })); }
...
function Harness() {
    ...
    const set = (s: Scenario) => { scenario = s; qc.invalidateQueries({ queryKey: ['avatars'] }); force(x => x + 1); };
    return (
        <div className="min-h-screen bg-[var(--blanc-bg)] p-6">
            <div className="flex flex-wrap gap-2">
                <Button onClick={() => setOpen(true)}>Open</Button>
                <Button variant="outline" onClick={() => set('connected')}>Connected</Button>
