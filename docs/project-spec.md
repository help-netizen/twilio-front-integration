...
### Technician day-off periods (TECH-DAYOFF-001)

An empty schedule used to look "free" to the engine, so Sara/VAPI, the outbound parts-visit robot and the Yelp agent booked customers onto days nobody worked. Day-off periods now live in `technician_time_off` (mig 167: `technician_id` = ZB team-member **TEXT** id, `starts_at`/`ends_at` timestamptz so a period can span midnight/days, `source individual|company`, `batch_id`, `created_by` crm_users.id) with CRUD at `/api/schedule/time-off` (`routes/schedule.js`): GET behind `schedule.view` (provider `assigned_only` → own rows only via the `company_user_profiles` bridge, deny-by-default), POST/DELETE behind `schedule.dispatch`; a company-wide entry materializes into K per-tech rows off the live ZB roster in one atomic INSERT (shared `batch_id`, ZB-fail → 502 and zero rows), deletion always per-row. Enforcement is an **A′ post-filter in `slotEngineService.getRecommendations`** — the single seam every consumer (Sara/VAPI `recommendSlots`, outbound robot, Yelp convo agent, slot-picker UI) already goes through: one SELECT over the horizon (0 rows ⇒ byte-identical legacy behavior), pre-shaping that drops a fully-covered tech from the roster, a `top_n` headroom of +5, rejection of recs overlapping a day-off (UTC, half-open intervals), then slice + re-rank. The slot-engine **container is untouched** — no container deploy needed. UI: a dispatch-only "Time off" button on Schedule opens a FORM-CANON panel (`TimeOffDialog`: technician|whole-company, from/to date+time, note ≤500, list of current/future entries with per-row delete); day-off ranges render as grey hatched blocks in Timeline/Week/Day views (pointer-events:none; mobile agenda gets a "Time off" card). Human overrides are **warning-only**: DnD onto a day-off, NewJobDialog and Job-card reschedule show confirms/inline warnings, Save is never disabled. +58 tests across 3 suites, 22 pre-existing suites drift-guard green; owner-gated deploy (mig 167).
...
The same skill layer also powers the multi-turn **Yelp email booking agent** (YELP-CONVO-BOOKING-001, LIVE prod; as of YELP-CONVO-CONTEXT-002 each turn's prompt carries a bounded untrusted-delimited transcript of the whole conversation and every agent send is linked onto the conv-id Pulse timeline — backend-internal, see `Docs/specs/YELP-CONVO-CONTEXT-002.md`).
...
## Schedule drive-time routes (SCHED-ROUTE-001 → SCHED-ROUTE-VIS-001)

The Schedule renders per-technician drive-time legs between a tech's same-day jobs: `route_segments` rows are reconciled per (technician, company-local day) by `routeSegmentService` (single recalc mechanism `recalcForJob`), computed asynchronously by an agentWorker `route_calc` task via `routeDistanceService` (global `route_calculation_cache`-first, Google Distance Matrix only on cache-miss, driving/no-traffic), and served by `GET /api/schedule/route-segments` (`schedule.view`, provider scope respected). SCHED-ROUTE-VIS-001 closed the coverage gaps that left segments mostly empty:

- **Recalc fires on every schedule-changing path**, not just schedule drag actions. Best-effort, non-fatal hooks (all delegating to `recalcForJob`) now cover: human job creation (`createDirectJob`, both ZB-success and local-fallback branches, plus a geocode enqueue for address-only jobs), the entire Zenbooker ingest (`syncFromZenbooker` — existing-row updates with a coords-changed delta so webhook echoes don't churn, create, and the delayed auto-assign re-fetch), and technician/date changes from the Job card (`POST /api/jobs/:id/reschedule`, capturing `beforeTechDays` before the ZB-assign so vacated days heal too). The pre-existing drag/address/geocode hooks are unchanged.
- **Lazy-on-read seeding self-heals history:** reading route-segments fire-and-forgets (`setImmediate`, response never waits) `seedMissingForRange` — one SQL detects up to 10 tech-day pairs in the requested range with ≥2 participating jobs but no active segments (or stuck `pending` ones), reconciles them, and enqueues computation through a deduplicated insert (`WHERE NOT EXISTS` on queued `route_calc` tasks). Provider scope carries through (`techFilter`); Google is never called in the HTTP path. A manual owner-run backfill script (`scripts/backfill-route-segments.js`) remains as a bulk alternative.
- **"Customer, City"** everywhere a job is listed: schedule API items carry `city`, schedule cards (classic and agenda layouts) and the desktop Jobs table's Customer column render `Customer, City` (city omitted when absent; mobile job cards already did this).

No migrations, no permission changes; the route-segments response contract and the `route_calc`/geocode handlers are untouched.
