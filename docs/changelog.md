...
Плеер записей переехал из карточки звонка в **плавающий hover-бар** внизу Pulse: Play/Pause (40px, фиолетовый акцент), −10/+10, подпись «номер · время звонка», **полноценная полоса прогресса с перемоткой** мышью/пальцем (её раньше не было вообще), время `тек/общ`, **скорость 1×→1.25×→1.5×→2×**, ✕. Карточка звонка стала чище: вместо кнопочного ряда — компактный чип Play/Pause + прежние тумблеры Summary/Transcript. Клик по строке транскрипта или entity перематывает ОБЩИЙ плеер и при необходимости переключает трек. **Только Pulse — по построению:** единственный `<audio>` живёт в `PulsePlayerProvider`, смонтированном в `PulsePage` (все /pulse-маршруты), поэтому уход на любую другую страницу размонтирует элемент и звук гарантированно останавливается; в jsdom это не докажешь — проверено в живом браузере (✕ убирает и бар, и `<audio>` из DOM). Бар — немодальный слой z-70 (ниже `OVERLAY_Z.panel` 80 — любой диалог его перекрывает); на мобиле прижат над нижней навигацией (`60px + 12px + safe-area`), ±10 там скрыты (перемотка драгом остаётся). «Ничего лишнего» (решение владельца): без download, waveform, запоминания скорости, глобального плеера; легаси-плеер `/calls` не тронут; live-транскрипция без изменений. Оркестрация БЕЗ GPT (прямое указание): полный пайплайн-цикл, реализация Claude. Тест-стиль дома (vitest env=node): решающее ядро вынесено в чистые функции + статический рендер бара — 12 новых тестов, фронт 27 файлов/150 зелёные, `npm run build` 0; в vitest.config добавлен алиас `@`→src (зеркало vite). Живой прогон в харнесе `/player-harness.html` (реальные PulseCallListItem + сгенерированный WAV): переключение треков из транскрипта, скорость, close, мобильная геометрия (наезд чипа на время пойман и починен `min-w-0` у range). **НЕ задеплоено.**
