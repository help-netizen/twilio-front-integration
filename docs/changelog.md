...
price-book комбобокс больше не закрывается на клике по категории (capture-фаза outside-click
+ reopen по клику); **канон MoneyInput** — маска денег 0.00 с набором справа (1→0.01→0.15→1.52)
во всех местах ввода сумм + глобальный запрет спиннеров; `%`-суффикс у ставки налога; новые
позиции по умолчанию облагаемые; авто-рост описаний (AutoGrowTextarea); редактор сметы —
одна колонка, Require signature/Deposit required плоско после Totals; панель сметы гидрирует
items (порт INVOICE-ITEMS-HYDRATE — «This estimate has no items» на здоровой смете исчез,
Send/Approve разблокированы); Summary в панелях как в редакторе; Preview PDF при блокировке
попапа скачивает файл с человеческим именем; **налог считается от (облагаемая база − скидка)**
(было: вся сумма × ставка — расходилось с превью редактора); **void ручных оплат** — миграция
197 (`payment_transactions.voided_at/voided_by` + бэкфилл legacy-событий), company-scoped
endpoint (manual-only, Stripe/ZB → 409, идемпотентно, аудит), voided исключены из
amount_paid/balance/финансов/аналитики, в UI серой строкой в конце списка с подтверждением.
Верификация: FE build + vitest 281; backend 20/20 (batch) + 24 + 200 (регрессии Codex,
перепроверены тимлидом); red-before/green-after на налоге; 4 диверсии red→restored. Спека:
`docs/specs/ESTINV-BATCH-001.md`. ⚠️ НЕ задеплоено; при деплое миграция 197 ДО образа.
...
## 2026-07-18 — TECH-SCHEDULE-001: персональные графики техников и зоны обслуживания

`/settings/technicians` теперь канонический экран активных техников: в строке сразу виден недельный график, а в правой панели настраивается персональная неделя с наследованием графика компании. Включённый чекбокс **Duplicate company schedule** показывает часы компании как read-only-зеркало; при выключенном наследовании для каждого дня задаются свои часы или **day off**. На открытый компанией день техник может работать шире общих часов — панель показывает предупреждение, — но закрытый компанией день остаётся абсолютным и не может быть открыт персональным графиком. Нерабочие интервалы вычисляются на чтении и **никогда не записываются в `technician_time_off`**: на technician-aware экранах Timeline, Team Week, mobile Day и Custom Time они используют тот же серый хэтч, что существующий time-off. Ручное бронирование не блокируется и получает только предупреждение, а smart-рекомендации за пределами персонального графика подавляются. В той же панели настраиваются зоны обслуживания по собственным данным Albusto: несколько ZIP-дистриктов или несколько радиусов; карты назначений двух режимов независимы и переживают переключение режима. Пустой набор в активном режиме означает wildcard «обслуживает всё», а Service Territories показывает отдельное постоянное, неснимаемое уведомление для каждого такого техника до первого валидного назначения. Назначения редактируются с обеих сторон — из настроек техника и из дистрикта/радиуса. Схема добавлена миграциями **183** (графики) и **184** (зоны обслуживания). **НЕ задеплоено.**
## 2026-07-19 — PULSE-PLAYER-001 (OB-13): плавающий плеер записей звонков в Pulse

Плеер записей переехал из карточки звонка в **плавающий hover-бар** внизу Pulse: Play/Pause (40px, фиолетовый акцент), −10/+10, подпись «номер · время звонка», **полноценная полоса прогресса с перемоткой** мышью/пальцем (её раньше не было вообще), время `тек/общ`, **скорость 1×→1.25×→1.5×→2×**, ✕. Карточка звонка стала чище: вместо кнопочного ряда — компактный чип Play/Pause + прежние тумблеры Summary/Transcript. Клик по строке транскрипта или entity перематывает ОБЩИЙ плеер и при необходимости переключает трек. **Только Pulse — по построению:** единственный `<audio>` живёт в `PulsePlayerProvider`, смонтированном в `PulsePage` (все /pulse-маршруты), поэтому уход на любую другую страницу размонтирует элемент и звук гарантированно останавливается; в jsdom это не докажешь — проверено в живом браузере (✕ убирает и бар, и `<audio>` из DOM). Бар — немодальный слой z-70 (ниже `OVERLAY_Z.panel` 80 — любой диалог его перекрывает). **Мобайл (доработка по фидбеку владельца в тот же день):** вместо тесного однострочного бара — **канонический BottomSheet** (~1/3 экрана): заголовок-подпись + ✕, крупная полоса перемотки во всю ширину с временами по краям, просторный транспорт −10 · Play 64px · +10 · скорость; свайп/фон = стоп. Иконки ±10 укрупнены и на десктопе (близко к размеру Play, «10» читается). Шит всегда смонтирован — шириной управляет только `open`: выдёргивание из дерева при повороте посреди воспроизведения пропускало close-хореографию и утекал body scroll-lock (поймано живой проверкой в харнесе); теперь при смене форм-фактора шит закрывается штатно, бар появляется, звук не прерывается. «Ничего лишнего» (решение владельца): без download, waveform, запоминания скорости, глобального плеера; легаси-плеер `/calls` не тронут; live-транскрипция без изменений. Оркестрация БЕЗ GPT (прямое указание): полный пайплайн-цикл, реализация Claude. Тест-стиль дома (vitest env=node): решающее ядро вынесено в чистые функции + статический рендер бара — 12 новых тестов, фронт 27 файлов/150 зелёные, `npm run build` 0; в vitest.config добавлен алиас `@`→src (зеркало vite). Живой прогон в харнесе `/player-harness.html` (реальные PulseCallListItem + сгенерированный WAV): переключение треков из транскрипта, скорость, close, мобильная геометрия (наезд чипа на время пойман и починен `min-w-0` у range). **НЕ задеплоено.**

---

## 2026-07-18 — CALL-BLACKLIST-001: чёрный список номеров — входящий отклоняется до маршрутизации

В **Настройки → Телефония → Blacklist** появился чёрный список телефонных номеров. Входящий звонок сверяется со списком **строго до маршрутизации по группам** — раньше кошелькового гейта, раньше `ingestToInbox` и раньше `resolveGroupForNumber` — и заблокированный абонент получает жёсткий `<Reject reason="busy"/>` **без ответа на звонок**, поэтому Twilio не запускает поминутный счётчик. Порядок проверки закреплён тестом через `invocationCallOrder`, а не только чтением кода. Список намеренно хранит **только номера**, без привязки к контактам (прямое требование владельца «просто номер телефона указывает и всё для простоты»), но в Pulse заблокированный звонок показывается с **именем контакта**, если номер кому-то принадлежит, и с новым значком блокировки вместо обычного «пропущенного» — статус звонка `blocked`. Заблокированные звонки сознательно **минуют inbox-воркер**: он по своей природе создаёт unread / Action Required / задачу, а именно этой работы блокировка и должна избегать; для них пишется отдельный снимок звонка. **Fail-open — центральный инвариант:** и сбой проверки, и сбой записи заблокированного звонка проваливаются в обычный маршрут, поэтому недоступность чёрного списка не может уронить телефонию; сервис специально не глушит ошибку внутри себя, её ловит вызывающая сторона. Миграция **185**: `company_id NOT NULL` + FK, per-company unique index на `(company_id, phone_e164)`, CHECK на US/CA E.164 (`+1` + 10 цифр — международные номера владелец из скоупа исключил), `created_by → crm_users.id`. Tandem: рамка, развилки и приёмочные гейты — Claude, discovery/UX/код/тесты — Codex (gpt-5.6-sol xhigh). Независимая проверка: 78 тестов в 5 профильных backend-сьютах, фронт 15 файлов / 87 тестов, `npm run build` exit 0; sabotage-контроль — убрал fail-open-обвязку, `TC-C-BL-02` покраснел, после восстановления снова 20/20. **НЕ задеплоено.**

---

## 2026-07-18 — LEADCALL-SMS-CANCEL-001: единая отмена робо-звонков после контакта с клиентом

Входящий SMS от клиента теперь отменяет запланированные ретраи робо-звонков **сразу для обоих outbound-агентов** — обзвона лидов и parts-visit; исходящий SMS ничего не отменяет, потому что это односторонний канал и он не доказывает ответ клиента. Причина отмены теперь попадает туда, где её действительно видит диспетчер: у лида — в `structured_notes` (лента Notes), у parts-job — в job note. До исправления триггер «клиент ответил на звонок диспетчера» вообще не оставлял заметку на лиде, а inbound-call писал причину в legacy `comments`, который экран Notes не читает, поэтому владелец этих объяснений не видел. Все три сигнала (`customer_answered_dispatcher_call`, `customer_called_in`, `customer_replied_by_sms`) сведены в один scenario-agnostic core `backend/src/services/outboundCallCancellationService.js`: он выбирает активные попытки по компании и телефону без scenario-фильтра, а frozen-карта `SCENARIO_HANDLERS` определяет только место заметки и scenario-specific side effects; поэтому следующая правка правил отмены сразу применяется ко всем outbound-агентам — это было прямое требование владельца. Попутно закрыты две ошибки изоляции: lead-cancel UPDATE не фильтровал `company_id` и мог отменить попытки для одинакового номера сразу в двух компаниях, а parts lookup не фильтровал `scenario='parts_visit'` и мог поглотить lead-attempt. Reason vocabulary между агентами унифицирован. Только backend, миграция не нужна. **НЕ задеплоено.**

---

## 2026-07-18 — AGENT-CALL-BADGE-001: звонки, принятые AI-агентом, отмечаются роботом в Pulse

Входящие звонки, которые принял AI-голосовой агент Sara, теперь показывают иконку робота вместо обычной стрелки входящего звонка **в обоих местах Pulse** — в боковом списке контактов и в ленте треда; направление при этом не теряется и остаётся доступным через `aria-label`/`title` (`AI answered · Incoming call`). Канонический маркер `answered_by='ai'` теперь сохраняется при propagation дочерней ноги в `in-progress` и восстанавливается при reconciliation родительского звонка; надёжное доказательство ответа агента — завершённая дочерняя SIP-нога на `*.vapi.ai`. Идемпотентная **миграция 180** бэкфиллит историю (на проде под предикат попадали **314 строк** на момент записи); rollback намеренно является no-op, потому что безопасно отличить уже существовавшие канонические маркеры от восстановленных невозможно. Выбор иконки вынесен в общий хелпер `pulseHelpers` и матчится **строго** по значению `ai`: прежний fuzzy substring-поиск удалён, поэтому человеческий SIP-username с буквами `ai` больше не может получить ложную иконку робота. В ранее нескоупленном query-path call-webhook добавлена фильтрация по компании. **НЕ задеплоено.**
## 2026-07-17 — UNKNOWN-CALLER-LEAD-001: голосовой агент больше не теряет постоянного клиента

Постоянный клиент звонил, система его УЗНАВАЛА (звонок ложился в таймлайн его контакта), но скилл `createLead` голосового агента эту идентификацию игнорировал: писал выдуманное «Unknown Caller» и не проставлял `contact_id`. Диспетчер видел незнакомца вместо человека с историей. Масштаб на проде: 53 из 62 лидов `AI Phone` без привязки (+10/10 `AI Phone (Invalid)`). Теперь `createLead` потребляет серверный `verifiedContext`: при однозначно опознанном звонящем в лид идут `contact_id` и НАСТОЯЩЕЕ имя из карточки. Fail-closed: привязка только при level L1/L2 + непустом имени + НЕ ambiguous + `phoneCandidateCount ≤ 1` — общий номер/несколько контактов/неопознанный → прежний «Unknown Caller» без привязки (лучше честный «неизвестный», чем заявка не тому клиенту). Безопасность УСИЛЕНА: для `createLead` модельный `contactId` вырезается ДО резолвера (`identityBlockFrom(..., {includeContactId:false})`) — модель не может подсунуть чужого клиента; правки `identityResolver`/`verificationGate` чисто аддитивные (новое `phoneCandidateCount`), инвариант AC-8 не тронут. Tandem: рамка+развилки+приёмка Claude, discovery/код/тесты Codex (gpt-5.6-sol). 363 теста в 25 сьютах зелёные; sabotage-контроль (пустить модельный contactId в резолвер) → тест краснеет. Спека: `docs/specs/UNKNOWN-CALLER-LEAD-001.md`. Долг: историческая правка 53 лидов и приоритет имени лида над именем контакта в Pulse.

---

## 2026-07-16 — TECH-DAYOFF-SLOTPICKER-001: time-off техника виден сразу в пикере «Pick a time»

При создании работы → «Pick time & provider» → «Pick a time» (`CustomTimeModal`) time-off техника раньше всплывал только предупреждением ПОСЛЕ подтверждения слота. Теперь он рисуется сразу в дорожке техника как **серый заштрихованный блок** (тот же диагональный хэтч, что в основном расписании DayView) с пунктирной рамкой, надписью **«Time off»** и периодом; блок неинтерактивный (`pointer-events:none`), поверх job-блоков. Модалка best-effort фетчит time-off на выбранный день (`fetchTimeOff({from,to})`, при ошибке просто пусто), группирует по технику. Геометрия клампится в рабочее окно (7–19): **многодневный time-off заполняет рабочие часы на каждый промежуточный день** («All day»), частичные дни — реальный интервал; полный период в `title`. Только фронт (`CustomTimeModal.tsx` + `.css`), без бэка (эндпоинт `/schedule/time-off` + provider-scope уже есть). Оркестрация: спека Claude (`Docs/specs/TECH-DAYOFF-SLOTPICKER-001.md`), код — Codex (gpt-5.6-sol xhigh), ревью + независимый build-гейт Claude (npm run build exit 0). Харнес time-off-фикстуры пока нет — визуальная проверка после деплоя.

---

## 2026-07-15 — TIMELINE-COMPANY-NUMBER-LEAK-001: SMS, привязанный к собственному номеру компании, «протекал» во все таймлайны

Расследование ложной тревоги «массовая rate-me рассылка всем клиентам». По данным прода массовой отправки НЕ было (2 rate-SMS за всё время, 1 тестовый разговор). Реальный баг — **утечка ОТОБРАЖЕНИЯ**: `discoverTimelineConversations` (paged) и `buildTimeline` (non-paged) в `pulse.js` собирают SMS-разговоры по набору телефонов, куда входят `callPhones` — ОБЕ ноги (`from_number`+`to_number`) звонков таймлайна. В каждом звонке одна нога = собственный номер компании (`+16175006181`), поэтому он всегда попадал в поиск, а один тестовый разговор, случайно закреплённый за `customer_e164 = <номер компании>` (rate-link по job 1443, где «клиентом» был номер компании), матчился и показывался в КАЖДОМ таймлайне со звонками. Фикс: общий хелпер `addExternalCallPhones` исключает собственные номера компании (`SELECT DISTINCT proxy_e164 FROM sms_conversations WHERE company_id=$1`) из call-ног перед поиском SMS — в обоих путях; собственный телефон таймлайна + secondary добавляются как прежде (нормальные таймлайны не затронуты). Регресс-тест TC-TRP-035 (legacy+paged). Оркестрация: спека Claude (`Docs/specs/TIMELINE-COMPANY-NUMBER-LEAK-001.md`), код — Codex (gpt-5.6-sol xhigh), ревью + независимый jest Claude (16/16). Бэкенд, без миграций. Тестовый «кривой» разговор безвреден после фикса.
...
- **Один дайлер, два сценария:** `outbound_call_attempts` получает `scenario` (DEFAULT `'parts_visit'`) + nullable `lead_uuid` + CHECK scope + partial-unique `uq_outbound_call_attempts_active_lead` — **миграция 173** (+ таблица `outbound_lead_call_settings`); **миграция 174** сеет тайл (идемпотентный ON CONFLICT-upsert, БЕЗ авто-install). Перенумерованы со спековских 172/173 — 172 занял `feedback_submissions` (renumber-recheck по правилу шапки плана). Parts-флоу **байт-идентичен** — заморожен golden-фикстурой wire-body (★TC-OLC-029, must-pass gate, ре-ран после каждого диффа worker/placeCall).
- **Триггер:** `leadsService.createLead` теперь эмитит fire-and-forget **`lead.created`** на eventBus (сразу после SSE `emitLeadChange`) — ОДНА точка покрывает ВСЕ пути инжеста (UI, integrations API, Yelp, Sara). Подписчик `'outbound-lead-caller'` гонит cheapest-first gauntlet: app connected → source enabled (нормализованный матч) → dialable phone (иначе `[AI Phone]`-trace в Comments) → goal-at-birth → lifetime-once → INSERT `('lead_call', attempt_no=1, 'pending', clampIntoWorkWindow)`. Side effect задокументирован: emit будит и automation_rules с `trigger_event='lead.created'` — pre-enable аудит в чек-листе деплоя.
- **Worker + webhook:** scenario-dispatch ветка в tick'е (`processLeadAttempt`: re-read → goal-achieved skip → eligibility re-check FR-15 → window-carry → слот-прекомпьют (нет слота → ladder `'no_slots'`, никогда не звоним с пустыми руками) → `placeCall` c lead-спредами + `firstMessage`-override) + lead-ветка в `vapiCallStatus` (booked-belt → declined по классификации/`structuredData.outcome` → transient → ladder/exhaustion). Лестница считается ТОЛЬКО в `scheduleLeadRetryOrExhaust`.
- **Букинг in-call:** новый L0-скилл `confirmLeadBooking` — identity ТОЛЬКО из инжектнутых variableValues (transport-аргументы не спуфят), **fail-closed offered-guard**: derived key == инжектнутый slotKey ЛИБО живая ре-валидация движком; hold-write byte-shape `bookOnLead` (`tzCombine`, coords both-or-nothing), БЕЗ job/нового лида/ZB; own-attempt flip → `'booked'` (CC-07: webhook становится no-op). В MCP НЕ экспонирован (voice-only).
- **Settings:** `GET/PUT /api/outbound-lead-caller/settings` (`tenant.integrations.manage`, tenant-изоляция по `req.companyFilter`) + страница: мультиселект источников (канон JOB_SOURCES ∪ наблюдённые company_sources, normalized dedup), «How it works», rollup «Last 30 days» строго по `scenario='lead_call'`.
- **Верификация:** 168 фичевых jest зелёные (7 новых сьютов) + 4 parts-сьюты нетронуты (zero-diff файлов); полный jest **3180 passed / 22 fails = пре-существующие** (вкл. параллельный дедлок marketplace .db-сьютов, ВОСПРОИЗВЕДЁН на базе 5f8290d без фичи — 0 новых регрессий; чип на фикс создан); **TC-OLC-057 real-PG 13/13** — поймал и исправил баг ON CONFLICT-inference предиката partial-index'а; **E2E dev-стенд 7/7** — живой `createLead`→eventBus→enqueue→3 ступени→exhaustion→FR-9 задача «slots unavailable»→lifetime-once.
- **Деплой (owner-gated, T10 готов):** psql 173+174 ДО кода → аудит automation_rules → **VAPI GET-before-PATCH** (prompt scenario-dispatch секция + tools `confirmLeadBooking`/`checkServiceArea` + `structuredData.outcome`; re-inject VAPI_TOOLS_SECRET; сверить serverMessages destination) → mirror-коммит → owner-смоук TC-OLC-063. До PATCH'а звонок поприветствует по firstMessage-override, но prompt-дискриминатор сценария и тулзы живут только в PATCH'е.
- Коммиты: 4b21e91 (docs-цепочка), c9cd355 (T1 миграции), 76ad73f (T2 settings-сервис), bfac8f8 (T3 emit+gauntlet), 1c1a60c (T4 worker+ladder), d453f2c (T5 placeCall+webhook), 3200a7f (T6 confirmLeadBooking), b5a60d6 (T7 роуты), a0ce176 (T8 страница), 898221f (live-stand фикс: index-предикат + category 'ai'), ee282ee (T9 verify record).
...
- **миграция 171** (index-only): partial `idx_calls_timeline_page` ON calls `(timeline_id, COALESCE(started_at, created_at) DESC, id DESC) WHERE parent_call_sid IS NULL` + rollback.
...
- Коммиты: f58ab04 (T1 + docs-цепочка), 0737b5e (T2 mig 171), 7a34f30 (T3 query-твины), bf786dd (T4 route+golden), fa83aac (T5 FE data layer), 6f90dab (T6 wiring), 7ca299f (T7 scroll-механика), 70570cc (T8 sticky AR), 1939390 (T9 харнес), 665a5e8 (T7 live-фиксы), 7e76e3c (TC-DO-29 un-freeze).
...
- **Миграция 166** `ALTER COLUMN lead_uuid TYPE text` (+ rollback). Идемпотентна; безопасна (пустая таблица, короткий код кастуется в text).
- **Применена как прод-хотфикс 2026-07-11** (таблица была пустая → мгновенно), эта миграция закрепляет для всех сред. Прод-лид «Kim» пере-поприветствован вручную; диалоговый агент теперь работает (приветствие с запросом телефона+адреса ушло на relay).
- **Follow-up (не в этом коммите):** усилить turn-0-хендлер — создавать разговор из task-input, если его нет (защита от любого будущего сбоя upsert). Сейчас не критично: колонка починена → upsert проходит.
...
- **Хук = событие `job.created` в eventBus** (не Postgres-триггер). Эмит аддитивно **post-commit** в `createDirectJob` (`jobsService.js`) и в `convertLead` под гардом `localJobCreated===true` (`leadsService.js`); оба через `.catch(()=>{})` — падение шины НЕ ломает создание работы. ZB-webhook-синк идёт через `createJob` (upsert) и заметок НЕ шлёт (вне scope Этапа 1). Тип `job.created` уже был в `eventCatalog`, но никем не эмитился — теперь эмитится.
- **Подписчик `kb-diagnostics`** (`eventSubscribers.js`): читает `event.company_id`+`event.payload.id`, **возвращается мгновенно** и кидает RAG-работу в `setImmediate` — иначе последовательный `await`-dispatch eventBus застопорил бы соседние подписчики на ~35с таймаута RAG.
- **`kbDiagnosticsService.runForJob({jobId,companyId})`** — вся логика, весь body в `try/catch` (никогда не бросает): gate `isAppConnected('ai-repair-advisor')` → `getJobById(jobId,companyId)` (company-scoped; чужой/удалённый ⇒ null ⇒ нет заметки) → идемпотентность (skip, если уже есть заметка `author==='AI Repair Advisor'`) → `buildQuestion` (пустой вопрос ⇒ stop) → `ragClient.ask` (null ⇒ stop) → `formatNote` → `addNote(jobId, text, [], 'AI Repair Advisor', 'system')`.
- **`ragClient.js`** — по образцу `zenbookerClient`: ленивый axios-синглтон, `retryRequest(fn,1)` (одна попытка, 4xx short-circuit). **Инертен по умолчанию** — `RAG_API_URL` пустой ⇒ 0 HTTP, `ask` возвращает null (нет hardcoded albusto-фолбэка в коде). Толерантный парс `/ask` → нормализованный `{summary,causes[],steps[],diagnosticMode,confidence,grounded}`; правило **empty⇒null** (нет причин И нет шагов И нет диаг-режима ⇒ null даже на HTTP 200 ⇒ нет заметки). Новые env `RAG_API_URL`/`RAG_TIMEOUT_MS` (default 40000) в `.env.example`.
- **Миграция 161** (`161_seed_ai_repair_advisor_marketplace_app.sql` + rollback) — структурная копия seed 126: gate-only (`provisioning_mode='none'`, `status='published'`, `app_type='internal'`, `requested_scopes='[]'`, без `setup_path`), идемпотентный upsert `ON CONFLICT(app_key)`. Зарегистрирована в `ensureMarketplaceSchema()` (иначе не применится в рантайме). Гейт = generic install-path (без спец-кейса в `isAppConnected`). № 161 подтверждён свободным во всех git-refs + воркри (садится в дырку master 160→162).
- **Верификация.** jest-свип **95/95 зелёный** — 46 фичевых (ragClient 16 / kbDiagnosticsService 30 / repairAdvisorEvents 12) + 49 регресс (jobsCreate/leadsService.convert/googleEmailMarketplace/marketplaceTelephonyOverlay/jobsService/pulseFinancialEvents). Запуск в воркри **требует** `--testPathIgnorePatterns "/node_modules/"` (корневой jest.config игнорит `/.claude/worktrees/` → без флага тесты молча скипаются). Фронт не тронут (0 FE-файлов в диффе). Независимое adversarial-ревью — **9 инвариантов держатся** (best-effort/offload/идемпотентность/изоляция/empty⇒null/scope), APPROVE-WITH-NITS, блокеров нет. **НЕ выполнено (среда):** живой up/down smoke миграции 161 — нет одноразового Postgres; помечено manual-before-deploy (валидировано статически против seed 126).
- `⚑ Деплой:` (1) связность Vultr→мини для RAG — владелец решает отдельно, **мини не трогать**; без `RAG_API_URL` агент инертен, фича не падает. (2) Ветка `mtech-phase0-deploy` на **86 коммитов позади master** (нет миграций 152–160, 162) → мержить/деплоить на **свежий master**, там прогнать свип заново. (3) Прогнать live up/down миграции 161 на staging/disposable PG до прода.
...
- **Уход из «Part arrived» любым путём** (ручная смена, FSM /apply, cancelJob, markComplete, ZB-cancel флип) → pending-попытки отменяются; живой звонок (`dialing`) не убивается, но получает маркер, глушащий retry-цепочку.
- **Успешный человеческий звонок** с клиентом (входящий или исходящий: final, completed, duration>0, answered) → план отменяется. НЕ отменяют: voicemail/no-answer/busy, собственные звонки робота (`answered_by='ai'`), звонки, отвеченные Sara (дискриминатор по call_flow_executions, fail-closed).
- **Каждая отмена = заметка к работе** («AI: robot call canceled — …» с причиной: какой статус / контакт по телефону) + штамп `canceled` на кнопке задачи (повторная постановка разрешена, штамп сбрасывается в `queued`).
- **Без воскрешения**: retry-вставки в вебхуке и воркере блокируются при уходе статуса / отменённой цепочке (`isChainCanceled` id-scoped — старые отмены не блокируют новую постановку); честный Guard-1 в воркере теперь пишет `canceled`+заметку вместо тихого `failed`.
...
**Суть:** исходящий part-arrived робо-звонок раньше жил только в `outbound_call_attempts` и не появлялся в звонковой ленте контакта. Теперь он зеркалится в `calls` как обычный звонок через один **не-фатальный** сервис `vapiCallTimelineService`: **строка-«placement»** в момент дозвона (синтетический sid `vapi:<callId>`, `direction=outbound`, `status=initiated`, `answered_by='ai'`, тред резолвится по телефону, company — только из attempt) → **живые mid-call переходы** → **финализация** с длительностью/исходом + записью + транскриптом + VAPI-саммари. Сбой хука НИКОГДА не роняет дозвон / вебхук-200 / retry-FSM.

- **Re-key синтетик→реальный sid:** как только VAPI отдаёт `phoneCallProviderId` (`CA…`), строка перекладывается на реальный Twilio-sid; коллизия с уже существующей строкой холодного реконсайла (23505) → merge-and-delete. Дочерние строки (transcript/recording) пишутся ТОЛЬКО после резолва sid (FK): VAPI-саммари → `transcripts.raw_payload.gemini_summary`, запись → `recordings.recording_url` (`source:'vapi'`).
- **Guard от убийства живого звонка:** 5-мин stale-sweep и hot-poll реконсайла получили `AND call_sid LIKE 'CA%'` → синтетическую `vapi:%`-строку Twilio больше не поллит (404→failed в середине разговора исключён); отдельный 15-мин sweeper добивает реально потерянную `vapi:%`-строку (`failed`, монотонно, company-scoped).
- **Проигрывание записи:** прокси `GET /:sid/recording.mp3` при не-Twilio sid стримит из `recordings.recording_url` (VAPI S3) без auth-заголовков; Twilio-ветка (`RE…`) байт-идентична — inbound / софтфон / Sara не затронуты.
- **Хуки:** воркер после штампа `vapi_call_id` зовёт `recordPlacement`; вебхук `/api/vapi/call-status` — `status-update`→`applyStatusUpdate` (раньше молча дропался) и `end-of-call-report`→`finalizeFromEndOfCallReport` (ДО attempt-FSM/idempotence-гейта, repeat-webhook safe), оба не-фатально; company — из коррелированного attempt (анти-спуф). Фронт: общий предикат `isAiAnsweredBy`, Bot-чип на call-тайле ленты (аддитивно; inbound Sara тоже помечается — так и задумано).
- **CT-07 (deploy-time, вручную, ещё НЕ применён):** PATCH живого OUTBOUND-ассистента `c1e2831b-…` — `serverMessages += status-update` (+ artifact/recording/transcript/summary в end-of-call), repo-JSON синхронизировать. Без него `status-update` просто не приходит (graceful degrade: end-of-call всё равно финализирует).
- **Проверка:** **107** backend jest зелёных в 6 сьютах (incl. red→green негативные контроли не-фатальности) + frontend `npm run build` зелёный; Reviewer **APPROVED**.
- **Non-gating notes:** (a) recording-прокси не имеет per-call company-SQL-скоупа — ПРЕД-СУЩЕСТВУЮЩЕЕ, идентично Twilio-ветке (tenant-чек в GET-гарде роутера); (b) двойной сбой (хук placement упал И `attempt.phone` пуст) → звонок есть в истории `/calls`, но не в треде Pulse (не-фатально, задокументировано); (c) у Bot-иконки дублируются `aria-label`+`<title>` (косметика).
- **Без миграции, без новых SSE-имён/роутов.** Фича deploy-safe при выключенном роботе (хуки просто не вызываются). Master-only, ещё **НЕ задеплоено**.

## 2026-07-09 — OUTBOUND-PARTS-CALL-TECHSLOT-001: робо-звонок предлагает окна ОДНОГО техника; 2+-tech джобы исключены; клиент может назвать день/время на звонке

**Суть:** пять требований владельца к part-arrived робо-звонку, все доставлены. **(1)** Джоб с **2+ техниками** — робот недоступен: не-обходимый сервер-гейт в `startRobotCall` → `{ok:false, reason:'multi_tech'}` (200-envelope, без INSERT/stamp), а `RobotCallSlotModal` вместо пикера показывает «…please call manually». **(2)** Робот предлагает окна выбранного техника: `techId` пикнутого лейна (dispatcher pick > sole-tech default > null) + `lat`/`lng` джоба едут в `slot_json` и инжектятся в `variableValues` звонка → in-call `recommendSlots` серверно ограничен этим техником (модель переопределить не может). **(3)** Рекомендации переноса на карточке джоба скоупятся на текущего техника (детерминированный первый по stable-sort id; таймлайны по-прежнему показывают ВСЕХ); REQ-3 guard: при 2+ техниках payload переноса **без** `tech_id` (time-only, оба остаются назначены). **(4)** Клиент называет день → `targetDay` (`YYYY-MM-DD` → earliest=latest) = окна этого дня. **(5)** День+время → `targetTime` (`HH:MM`) возвращает **ровно одно** окно: containment → dist 0, иначе `argmin|start−T|`, tie → раньше.

- **Слот-движок НЕ менялся** — вся формовка входа в backend-прокси `slotEngineService` (опциональный `new_job.technician_id` фильтрует ростер компании до одного техника; unknown id → пустой ростер → safe-fail; single-tech запрос расширяет ranking-капы `top_n`≥5 / per-technician / per-timeframe = 5 через merge `config_override`) + скилл `recommendSlots` (невалидные форматы игнорируются, safe-fail не тронут). Легаси-путь байт-идентичен.
- **Фронт:** `RobotCallSlotModal` — multi-tech панель + `techId` в POST + обработка серверного `multi_tech`; `CustomTimeModal` + `slotRecommendationsApi` — аддитивный `recommendTechId?` → `technician_id` в запросе рекомендаций; `JobInfoSections` — скоуп переноса + REQ-3 guard.
- **VAPI:** repo-половина — `parts-visit-scheduler.json` tool-schema += `targetDay`/`targetTime` + prompt-гайд (`technicianId` НЕ добавлен — только server-injected). **Live PATCH ассистента = ручной шаг деплоя, ещё НЕ применён.**
- **Проверка:** backend jest **294** зелёных в **14** сьютах (вкл. новый `tests/recommendSlots.test.js` и red→green негативные контроли); заодно починены предсуществующие протухшие голдены f73636d (старый сокращённый slot-label) в `vapi-tools.test.js` / `golden.json` / `agentSkillsLegacyGolden`; frontend `npm run build` зелёный. Reviewer **APPROVED**.
- **Non-gating notes:** (a) `initialSlot` в `JobInfoSections` берёт несортированный `assigned_techs[0]`, а `recommendTechId` — sorted[0] → на 2+-tech джобах возможен косметический mismatch подсветки лейна; (b) расширение капов действует и на десктопные single-tech рекомендации (консистентно со спекой).
- **Без миграции, без изменения движка.** Master-only, ещё **НЕ задеплоено**.
## 2026-07-10 — GMAIL-PUSH-FIX-001: восстановлен real-time Gmail push (cursor-баг + 10-мин poll + наблюдаемость)

**Суть:** входящая почта снова доставляется за секунды, а не ждёт ~10-минутного поллинга (латентность одиночного письма падала до ~571с). Три backend-only фикса, без миграции и без изменений в Google Cloud. (1) **cursor-баг** — `GmailProvider.handlePushNotification` возвращал `cursor:` из пушнутого historyId; но пушнутый Gmail `historyId` уже включает triggering-сообщение, поэтому `history.list(startHistoryId=этот)` для одиночного письма пуст → пуш впустую. Теперь возвращает `cursor: null`, и `pullChangesNormalized` идёт от сохранённого поллингом `mailbox.history_id`, где новое письмо в окне → ингест+линковка за секунды. (2) **10-мин poll-баг** — `emailQueries.listDueMailboxes` overlap-guard был захардкожен `last_sync_started_at < now()-interval '10 minutes'`, дросселируя fallback-поллинг до ~10 мин независимо от `EMAIL_SYNC_INTERVAL_MS`. Новый предикат: каденция от последнего FINISH честно уважает интервал, overlap блокируется только для реально идущего синка, 10 мин остаётся stuck-escape; плюс экспортирован чистый `isMailboxDue(row,{intervalMinutes,now})`, зеркалящий SQL для юнит-тестов. (3) **наблюдаемость** — один success-лог `[EmailPush] push handled` в `emailTimelineService.ingestPushNotification` (успех пуша нигде не логировался → ложный диагноз «push мёртв» 2026-07-06).

- **Файлы:** `backend/src/services/mail/GmailProvider.js` (FIX#1), `backend/src/db/emailQueries.js` (FIX#2 + экспорт `isMailboxDue`), `backend/src/services/email/emailTimelineService.js` (FIX#3). Тесты: `tests/mailProvider.test.js` (TC-ET-040 перевёрнут), новые `tests/emailDueMailboxes.test.js` + `tests/emailPushIngestLog.test.js`; новый `scripts/verify-gmail-push-fix-001.js` (deploy-time live-push харнесс).
- **Проверка:** backend jest **48** зелёных (sabotage-контроли load-bearing) + deploy-time live-push харнесс для прод-верификации.
- **Без миграции, без изменений в Google Cloud.** Master-only, ещё **НЕ задеплоено** (деплой по «да» владельца).
...
- **Фикс 1** (`addressAutoHelpers.stateFromZip` + эффект в общем `AddressAutocomplete`): когда адрес приходит с валидным 5-значным ZIP, но пустым штатом — выводим штат из ZIP по 3-значному префиксу для Новой Англии (MA 010–027, RI 028–029, NH 030–038, ME 039–049, VT 050–059, CT 060–069). Неизвестный префикс → `''` (НИКОГДА не угадываем неверно; вне региона выбираем вручную). Заполняет только когда пусто → не спорит с ручным выбором. Централизованно в `AddressAutocomplete` → чинит New Job, Edit Lead, Contact, Convert-to-Job разом. `stateFromZip` проверен 12/12 (01721→MA, NY/CA→'').
- **Фикс 2** (`ui/BottomSheet` body): добавлен `overscroll-behavior: contain` на скролл-контейнер шторки — тач-скролл остаётся ВНУТРИ списка и не утекает в форму позади. Общий фикс — чинит скролл ЛЮБОГО длинного мобильного Select (штаты и др.).
- **Проверка:** `npm run build` зелёный; `stateFromZip` 12/12. Тач-скролл на iOS десктоп-preview не воспроизводит faithfully → финальная проверка на iPhone владельца после деплоя. Frontend-only, без миграций.

## 2026-07-07 — OUTBOUND-PARTS-CALL-001: исходящий VAPI-агент планирует визит завершения после прибытия запчасти (+ TASK-ACTIONS)

- **OUTBOUND-PARTS-CALL-001:** когда работа переходит в новый статус **«Part arrived»** (FSM мигр 156), авто-создаётся задача `kind='part_arrived_call'` с **двумя кастомными кнопками** (типизированные task-actions): «🤖 Робот звонит» и «📞 Позвоню сам». Исходящий голосовой агент дозванивается клиенту и бронирует визит завершения ремонта.
- **TASK-ACTIONS (переиспользуемый под-компонент):** `tasks.actions` jsonb (мигр 157) + закрытый серверный реестр (`robot_call`/`manual_call`) + `POST /api/tasks/:id/actions/:type` (`tasks.manage`, company-scoped). Фронт (`TaskCard.tsx`) рендерит кнопки из `actions[]` рядом со штатными Готово/Cancel — никаких hardcoded per-feature кнопок.
- **robot_call:** заранее считает слот (`recommendSlots`) и кладёт в очередь `outbound_call_attempts` (мигр 158, partial-unique `(job_id)` — один активный звонок на работу); нет слотов / ошибка движка / нет телефона → **НЕ звонит**, причина + действие для диспетчера пишутся в задачу. Воркер (env-gated, business-hours clamp по tz компании, backoff immediate/+2ч/след. раб. утро ×3, claim `FOR UPDATE SKIP LOCKED`) звонит через VAPI `/call` с контекстом в `assistantOverrides.variableValues`; каждая неуспешная попытка = AI-Phone заметка в работе; после 3-й — задача остаётся диспетчеру.
- **Во время звонка** ассистент (новый `parts-visit-scheduler.json`, НЕ подтверждает имя/адрес, без гарантийной фразы) предлагает готовый слот; отказ → `recommendSlots` вживую; согласие → `confirmPartsVisit`: reschedule **ТОЙ ЖЕ** работы (ZB write-through) + статус → Rescheduled + AI-Phone заметка + задача Done. **Нового лида/работы НЕ создаётся.**
- **Результат звонка** классифицирует secret-auth webhook `POST /api/vapi/call-status`: company берётся **из attempt-строки** по `vapi_call_id`, НЕ из body (анти-спуф); booked/no_answer/voicemail/declined + retry/exhaustion. **manual_call** → софтфон (`openDialer` на десктопе / native `tel:` на мобиле — MOBILE-NO-SOFTPHONE-001).
- **Безопасность:** `confirmPartsVisit` — L0 на исходящей поверхности (пред-связанный контакт) с ownership-пречеком `job.contact_id===bound contactId`; identity инъектится сервером через `variableValues` (спред последним — модель не может её подменить); company-scoped везде; v1 гейт на Boston Masters. Inbound Sara/`vapi-tools` байт-идентичны (variableValues no-op без assistantOverrides).
- **Верификация:** **87** jest + real-DB harness `scripts/verify-outbound-parts-call-001.js` **13/13** (FSM-мигр, идемпотентность задачи, partial-unique дубль, ZB-409, cross-tenant, fail-safe хук; sabotage-контроль ×2). Harness поймал **2 бага**, не ловимых jest-моками: (1) `tasks_kind_check` не допускал `part_arrived_call` → авто-задача молча не создавалась; (2) `robot_call` читал `task.job_id` вместо `parent_type/parent_id` → всегда `not_dialable`. Оба починены. Reviewer APPROVED.
- **Деплой owner-gated:** миграции 156–159, VAPI env (`VAPI_API_KEY`/`VAPI_OUTBOUND_ASSISTANT_ID`/`VAPI_OUTBOUND_PHONE_NUMBER_ID`, опц. `VAPI_WEBHOOK_SECRET`) + пуш ассистента в VAPI + `FEATURE_OUTBOUND_CALL_WORKER=true`. **Fail-safe: сбой хука/сервиса НЕ роняет переход статуса.** Перед прод — прогон harness на prod-copy. **Master-ready, NOT deployed.**

## 2026-07-07 — STRIPE-ADHOC-PAY-001: приём Stripe-оплаты с произвольной суммой прямо из вкладки Финансы джоба (orchestrate-пайплайн)

Раньше веб-приём Stripe жил ТОЛЬКО внутри панели инвойса (нужен инвойс с остатком) — на уровне вкладки Финансы джоба кнопки не было, произвольной суммы тоже. Теперь на карточке работы → вкладка Финансы (рядом с блоками Estimate/Invoice) есть кнопка **«Collect payment»**: вводишь произвольную сумму → **списать карту вручную** (Payment Element) / **отправить ссылку** (email/SMS клиенту) / **скопировать ссылку**. Платёж = standalone на джобе (`job_id`, БЕЗ инвойса). Когда Stripe не подключён — на месте кнопки CTA-заглушка «Connect Stripe…». Полный /orchestrate (агенты 01–08, auto-run, 5 задач SAP-01..05). **Без миграции** (колонки `job_id` + права `payments.collect_*` уже есть; max миграции 155). Надстройка над F018/STRIPE-PAY-001 (Фазы 1–5), инвойс-путь не тронут.

- **Backend** (`stripePaymentsService.js`): `assertAdhocAmount` ($0.50–$100k, 2dp, NaN/neg/zero→INVALID_AMOUNT 400) + `ensureJobPaymentLink`/`getJobPaymentLink`/`sendJobPaymentLink`; `resolveSurfaceContext` получил `else if (jobId)` (lazy `jobsService.getJobById` → contact, company-scoped 404, прогон assertAdhocAmount) — инвойс-ветка байт-в-байт цела. Отправка зеркалит `invoicesService.sendInvoice`: email `emailService.sendEmail` (409 MAILBOX_NOT_CONNECTED), SMS `resolveCompanyProxyE164`+`conversationsService.sendMessage` (422 NO_PROXY/NO_PHONE, 402 WALLET_BLOCKED); нет контакта→422 NO_CONTACT; ссылка идемпотентна `job-${companyId}-${jobId}-${amount}`, surface `checkout_link` (invoice_id NULL). Queries `findOpenJobSession`/`listSessionsForJob`. Роуты `routes/jobs.js`: POST/GET `/:id/stripe-payment-link` + POST `/:id/send-payment-link` (гейт collect_online/view, company-scope). Платёж в `payment_transactions` с `job_id` через СУЩЕСТВУЮЩИЙ вебхук — вебхук/леддер не менялись.
- **Frontend**: `CollectPaymentDialog` (FORM-CANON; сумма-префилл остатком; валидация зеркалит assertAdhocAmount; 3 способа). `JobFinancialsTab` кнопка/CTA: гейт сперва по правам, затем `can_collect` (покрывает connected_ready И payouts_disabled); не-ready+право → CTA на `--blanc-surface-muted` («Accept payments right from the job» / «Finish your Stripe setup…»; без `tenant.integrations.manage` → «Ask an account admin…»). Обобщён `ManualCardDialog` на `{jobId?,invoiceId?,amount}` (инвойс-call-site цел). `jobStripeApi`. Публичная `/pay/thanks` объявлена ДО `/pay/:token` (иначе `:token="thanks"` → инвойс-страница 404); ad-hoc Checkout = Stripe-hosted, редиректит сюда.
- **Пойман пайплайном:** Reviewer нашёл реальный баг фронт↔бэкенд — «Send payment link» слал запрос БЕЗ суммы → `assertAdhocAmount(undefined)` → INVALID_AMOUNT 400 для каждого джоба с контактом (send был бы сломан в проде); 42 unit-теста пропустили (все слали amount явно). Фикс: `jobStripeApi.sendLink` получил amount + `CollectPaymentDialog` его шлёт; TC-SEND-11/12 добавлены.
- **Проверка:** backend jest **72/72** (44 adhoc + 28 инвойс байт-в-байт — AC-5), `npm run build` exit 0, sabotage кусается, static-гейты (route order, max mig 155, инвойс call-site). Артефакты: requirements/architecture + spec/test-cases + план.
- **⚑ Осталось (owner):** чтобы РЕАЛЬНО списывать — Stripe Connect должен быть `connected_ready` (Settings→Integrations→Stripe Payments), иначе видна CTA-заглушка. MANUAL на подключённом аккаунте + деплой — owner-gated. **Ещё НЕ задеплоено** (commit 879a501).
...
## 2026-07-07 — PWA-FIX-001: установленная PWA Albusto на iPhone перестаёт выбрасывать навигацию в SFSafariViewController (orchestrate-пайплайн)

На iPhone установленная на домашний экран PWA (`display:standalone`) стартовала корректно, но при переходе по внутренним ссылкам iOS **выбрасывал навигацию из standalone-окна в наложенный in-app браузер SFSafariViewController** (крестик сверху, тулбар снизу, другие safe-area-инсеты → вёрстка «едет»). Это НЕ проблема брейкпоинтов и не воспроизводится уменьшением окна десктопа. **Причина (проверена на живом проде):** у фронтенда вообще НЕ было Web App Manifest (следовательно нет `scope` — iOS считает областью только стартовый путь) + логин Keycloak делал полный **кросс-origin** редирект `kc.login()` на `auth.albusto.com` при ЛЮБОМ сбое рефреша токена (даже транзиентном сетевом) → выброс в in-app браузер и застревание в нём. Полный прогон /orchestrate (агенты 01–08, auto-run, 6 задач PWA-01..06). **Только фронтенд, без бэкенда, без миграции.**

- **Web App Manifest** `frontend/public/manifest.webmanifest` — **`scope:"/"`** (главный фикс: все маршруты in-scope, навигация больше не считается out-of-scope), `start_url:"/"`, `display:"standalone"`, name/short_name «Albusto», `theme_color`/`background_color` `#fffdf9` (тёплый почти-белый, сливается со статус-баром), icons 192/512/512-maskable.
- **`index.html` `<head>`** — `<link rel=manifest>`, `apple-mobile-web-app-capable=yes` (+ новый `mobile-web-app-capable`), `apple-mobile-web-app-status-bar-style=default`, `apple-mobile-web-app-title=Albusto`, `theme-color`, `apple-touch-icon` 180, `viewport-fit=cover` (корректные safe-area в standalone).
- **Бренд-иконки** `frontend/public/icons/` — леттермарк «A» (near-white `#fffdf9`) на скруглённой ink-плашке `#030213`, палитра Т2: `icon-192`, `icon-512`, `icon-512-maskable` (full-bleed, ≥20% safe-inset), `apple-touch-icon-180` (full-bleed, iOS сам скругляет) + 2 SVG-источника. Растрированы Pillow'ом (librsvg не встал — брю-права), PNG пиксель-точные и закоммичены (прод-Docker не требует librsvg).
- **Auth-фикс** — `frontend/src/auth/refreshPolicy.ts` (ЧИСТЫЙ классификатор `classifyRefreshFailure`: offline⇒transient short-circuit, нет refresh-токена⇒dead, `invalid_grant`/«session not active»/«token expired»/«refresh token»⇒dead, ambiguous⇒transient — bias «никогда не выбрасывать на неоднозначности»; backoff `[2s,5s,10s]`) + `AuthProvider.tsx`: оба `.catch`-сайта (interval 30с + `onTokenExpired`) через один `refreshTokenOrLogin` — на транзиентном сбое retry с backoff, полный `kc.login()` ТОЛЬКО когда сессия реально мертва или исчерпаны ретраи (~17с). Два защищённых no-session `kc.login()` (401/403-мост, security-fallback) не тронуты.
- **Пойман пайплайном:** Reviewer нашёл и починил регрессию happy-path — оркестратор с `|| attempt===0` гонял бы `/api/auth/me` (fetchAuthzContext) каждые 30с на живом токене (оригинальный interval этого не делал); фикс = `if (refreshed) onRefreshed()`, поведение при живом токене снова байт-в-байт.
- **Проверка:** `npm run build` ЗЕЛЁНЫЙ (tsc -b + vite, прод-строгий noUnusedLocals; предупреждения о чанках — пред-существующие), `dist/` содержит manifest+icons+head-теги; `node frontend/scripts/verify-pwa-refresh-policy.mjs` — 30/30 ассертов (8 truth-table + extraction + backoff + **sabotage-контроль** + drift-guard, читающий реальный `.ts`). MANUAL iOS-проверки (install/standalone/navigate-no-eject/safe-area) — owner-gated.
...
- T9 owner-итерация (GPT, ACCEPT): икон-плитки шагов в /welcome (Receipt/Phone/Mail/CreditCard на var(--blanc-accent-soft), fallback Sparkles), done-галочка к заголовку, Circle-маркер удалён.
- T10 owner-итерация (GPT, ACCEPT): шаг company_profile → «Complete your company profile» (посыл: профиль показывается в эстимейтах/инвойсах); done ⇔ logo AND city AND state AND zip; est 2 min; спека §1.2 обновлена.

## 2026-07-13 — BUG-22/22b: вечная петля «Confirm it's you» (verify phone) убита

Инцидент: тест-учётка с verified phone на недоверенном iOS-устройстве уходила в бесконечный цикл перезагрузок с SMS-запросом. Слой 1 (373ec40): axios-клиент научен PHONE_VERIFICATION_REQUIRED → 2FA-гейт (не session-expired). Слой 2 — КОРЕНЬ (BUG-22b): kc.init падает на iOS (обрыв token-exchange) → незащищённый fallback kc.login() + живой SSO = вечный редирект-цикл (~600мс), гейт ремаунтится, otp/send на каждом витке (SMS-throttle удержал: 2 SMS за весь инцидент). Fix: auth/loginLoopBreaker.ts — кросс-релоадный ограничитель (≤2 форс-редиректов/30с через sessionStorage) на все 3 пути к kc.login() (fallback init, session-expired, refresh-dead), сброс при успешном init, fatal-экран «We couldn't sign you in» + Try again. ВОСПРОИЗВЕДЕНО и проверено руками на стенде (реальный фронт + мок-OIDC + стаб-API 401 PHONE_VERIF, scratchpad stub-2fa): loop-режим — ∞→3 редиректа и стоп/0 SMS; healthy-режим — стабильный гейт, ровно 1 SMS. Тесты: +4 vitest loginLoopBreaker (обязательные), итого 7/7. LESSON: ручной прогон фичи с реальным 2FA-состоянием обязателен — preview на dev-БД этот путь не покрывал.
- T10 owner-итерация (GPT, ACCEPT): шаг company_profile → «Complete your company profile» (посыл: профиль показывается в эстимейтах/инвойсах); done ⇔ logo AND city AND state AND zip; est 2 min; спека §1.2 обновлена.

## 2026-07-13 — SERVICE-TERR-002: два режима территорий (zip-list / радиусы) + карта + мобила + онбординг-шаг

GPT-implementer (gpt-5.6-sol): 5/5 задач ACCEPT (2 через plan-first). миграция 171 (company_territory_settings, territory_radii со снапшотом координат, глобальный zip_geocache — документированное исключение), haversine-утилита, territoryGeoService (кэш-first Google geocoding, без negative-cache), ЕДИНЫЙ containment-seam territoryService.isZipInTerritory (list=прежний search байт-в-байт; radius=геокод+haversine к ближайшему покрывающему кругу) — на seam переведены zip-check и Sara-скилл checkServiceArea (frozen-шейпы, golden зелёные). API /config (lazy-seed центроидов cap 10) /mode /radii. Страница: toggle режимов (данные обоих живут независимо), radius-CRUD (Base-бейдж, 422 ZIP_NOT_FOUND), read-only Google-карта (Circle/Marker, gestureHandling none), мобильная вёрстка починена. Онбординг-шаг company_profile → service_territory («Set up your service territory», MapPin, done по активному режиму). Верификация: 116+ jest по задачам, preview desktop+375 с live-прогоном seam (радиус-containment подтверждён), полный jest 57/57 после закрытия двух унаследованных дрейфов (TC-DO-29 → 168; TC-EO-U09 → display_name/external_source из b063142, недообновлённый point-deploy — доказано diff'ом master). НЕ задеплоено (deploy-consent).

## 2026-07-13 — TELEPHONY-WIZARD-UX-001: визард телефонии по беклогу владельца (OB-1..5, OB-7)

GPT-implementer: 4 волны ACCEPT (T2/T3 plan-first). T1: $5 welcome-кредит (walletService, ref welcome_credit:v1, real-PG тест гонки) + payg-автоактивация с trial + ленивый connect в search/buy + очеловеченный Stripe-чеклист (test_payment→first_payment «Start getting paid…»). T2: Twilio Port-in backend (мига 169 port_in_requests c partial-UNIQUE активных, DB-first ordering, portInService на проверенном SDK 5.12 master-scope, LOA signature URL, PII не наружу; capital-D /v1/Documents по актуальным докам). T3: визард 3 шага→2 (Plans опционален: Skip; баннер «You have $5…»; Keep this plan вместо блокировки; AreaCodeCombo — one-field код/город, NANPA-справочник ~350 кодов, локальные первыми от companies.city/state/zip; серый sectionCard убит). T4: Transfer-панель (Check if it can move → форма → статусы/LOA/cancel; рекомендация владельца дословно) + Stripe-экран single-hero (WhatItCostsCard удалён). Верификация: 43+59 jest, 24 vitest, полный jest 0 новых регрессий, preview 1280/375. Уроки: L-014 (NODE_USE_SYSTEM_CA=1 сегфолтит Node25 — причина Vite-крашей). OB-5 остаток — проверка на устройстве после деплоя. НЕ задеплоено.

## 2026-07-13 — YELP-CONVO-CONTEXT-002: Yelp-агент помнит всю переписку (bounded транскрипт в промпте) + его ответы видны в Pulse-таймлайне

**GPT-implementer-прогон: код писал Codex gpt-5.6-sol, Claude — архитектор/ревьюер; 8/8 задач ACCEPT, 1 clarification-stop (T6: порядок markGreeted→link), 0 fix-раундов.** Два owner-аска: «агент не теряет контекст?» и «отправленные ответы не выводятся в таймлайне». Backend-only: НИ миграций, НИ новых таблиц/колонок/роутов, НИ фронта; dark-safe (Phase-A ack байт-идентичен, гейты `YELP_CONVO_ENABLED`/`YELP_AUTORESPONDER_ENABLED` не тронуты).

- **(A) Транскрипт в промпте:** каждый ход `runTurn` получает bounded хронологическую переписку разговора (обе стороны, включая забаунсенные Yelp'ом отправки) из `email_messages`. NEW pure-модуль `backend/src/services/yelpConvoHistory.js` (реюз `emailTimelineBody.toTimelineBody`; caps 600/entry · 6000/total · 30 msgs; drop-oldest-first + маркер «(earlier messages omitted)»; invisible-char strip; `"{3,}`→`""` — fence-break невозможен) + NEW `emailQueries.listYelpConversationHistory` (одна company-scoped SQL: timeline-linked OR thread-sibling outbound с draft-дискриминатором; exclude текущего inbound по bare-pmid THREADING-002-формы). `runTurn` стэшит `conv.__timelineId`/`__history` рядом с `__threading` (threading-SELECT аддитивно отдаёт `timeline_id`); `buildPrompt` вставляет блок CONVERSATION SO FAR между OFFERED SLOTS и CUSTOMER MESSAGE под UNTRUSTED-DATA-постурой (SECURITY-строка расширена); knobs `YELP_CONVO_HISTORY_MAX_CHARS/ENTRY_CHARS/MAX_MESSAGES`; двойной fail-open (fetch/compose-фолт → сегодняшний no-history промпт, ровно ОДИН send); D1-лог на ход.
- **(B) Отправки агента на таймлайне:** NEW `emailTimelineService.linkYelpAgentSend` — post-send best-effort линк отправленного письма на conv-id таймлайн (contact_id NULL — LOAD-BEARING для mail-mute-гарда `email_by_timeline`; timeline-keyed idempotency без SSE-спама; no-row → reimport → retry once; SSE `publishMessageAdded`; unread/AR/контакты НЕ трогаются). Вызывается из хвоста `sendOnce` СНАРУЖИ `__sendFault`-поверхности (все терминалы, вкл. turn-0 и catch-fallback; link-фолт никогда не ретраит задачу и не дублирует письмо) + шаг 5b гритера `yelp_lead` после `markGreeted`; D2-лог на send. Плюс owner-run backfill `backend/scripts/yelp_agent_sends_backfill.js` (dry-run default, `--apply --yes`, snapshot-first, конфликт-тред → skip+`residueOutbound`, идемпотентен; на прод — scp + docker cp).
- Коммиты: cadb540 (docs-chain) → 1c18563 (T1) · d59364e (T2) · 421c103 (T3) · f30c0fd (T4) · 8db77b1 (T5) · 63b151b (T6) · ac41c55 (T7); T8 = final verify без кода.
- **Тесты:** 54/54 TC замаплены и прогнаны; финальный gate **35 suites / 417 tests** зелёные (вкл. real-PG db-сьюты); 5 именованных sabotage-контролей red→revert; исходные 484 строки loop-сьюта byte-unchanged. **НЕ задеплоено** — и деплой, и прод-прогон backfill-скрипта только по явному «да» владельца (deploy-consent).

## 2026-07-13 — MARKETPLACE-LEADGEN-SPLIT-001: «Lead Generator» разделён на 5 per-source приложений каталога

- **Rename:** `lead-generator` → **Website Leads** (app_key/инсталляция/живой токен нетронуты); **new:** Pro Referral Leads, Rely Leads, NSA Leads, LHG Leads (provider Albusto, lead_generation, `leads:create`, published) — миграция **169** + boot-list регистрация после 083 (иначе boot-ресид молча откатывал бы имя).
- **Default-компания:** новые приложения авто-connected, шарят live-credential через CROSS JOIN LATERAL resolve (без хардкода id); сид NOT-EXISTS по всем статусам (boot-реплеи не дублируют/не воскрешают).
...
## 2026-07-13 — CLIENT-FEEDBACK-WIDGET-001: виджет обратной связи (юзер CRM → разработчик Albusto)

GPT-implementer: 2 волны ACCEPT (backend+frontend), 0 fix-раундов. T1 backend: миграция 172 feedback_submissions (tenant-isolated, источник истины) + feedbackQueries + feedbackService (INSERT-first + best-effort письмо на support@albusto.com через платформенного отправителя FEEDBACK_SENDER_COMPANY_ID; email-сбой не роняет INSERT, meta.email_status) + POST /api/feedback (authenticate+requireCompanyAccess, multer memoryStorage, валидация→422); 30 jest. T2 frontend: FeedbackWidget (floating-кнопка внизу-справа под softphone; Messenger-панель; детерминированный бот-заглушка greeting→chatting→escalated, эскалация по «Talk to a human» или botReplies>=2; форма email(prefill useAuth)+What happened?+файлы; multipart submit); монтаж в AppLayout за VITE_FEATURE_FEEDBACK_WIDGET; 39 vitest. Верификация: preview desktop+375 (кнопка→чат→эскалация→форма с prefill dev@localhost→mobile, не перекрывает нижнюю навигацию/softphone). AI-агент первой линии и «обратная сторона» — отдельной волной позже (сейчас бот = заглушка, обращения уходят письмом + в БД). НЕ задеплоено.


## 2026-07-13 — RATE-ME-CRM-001: Rate Me стал мультитенантным сервисом внутри CRM (инфра-фаза)

- **Данные (миграция 172):** rate_tokens (192-бит непереборный токен, company-scoped, job/tech-привязка), technician_ratings (UNIQUE rate_token_id = идемпотентность, stars 1-5), rate_me_domains (UNIQUE domain + UNIQUE company, lifecycle pending→verified→active/failed) + нормализация hand-inserted rate-me app-ряда.
- **Публичная страница:** app-SPA роут /r/:token, обслуживается на rate.albusto.com И кастом-доменах тенантов через **rateHostGate** (Albusto-хосты = zero-DB pass-through, весь CRM цел; rate-хосты = allowlist /r|/api/public/rate|assets → 404 на всё остальное, никакого CRM/KC/webhook; кастом-хост fail-closed 503). Публичный API /api/public/rate (GET-контекст 60/min + POST-оценка 10/min XFF, GET rate-domain-ask для Caddy on_demand_tls, loopback-guard, 200/404 silence). Мобайл-первый RatePage: 5★→Google-редирект, 1-4★→фидбек→спасибо; identity ВСЕГДА из токена, не из тела.
- **Кастомный домен:** настройки Rate Me (радио albusto|custom + человечная форма: показываем CNAME-запись, Verify через dns.resolveCname, чипы статусов, google_review_url); Caddy on_demand_tls ask-эндпоинт выдаёт серт ТОЛЬКО verified/active доменам; infra/Caddyfile-фрагмент + README.
- **Изоляция (святое):** непереборные токены, доменная привязка 1:1, ask-silence, per-app settings-dispatch (rely-leads байт-в-байт).
- Пайплайн: 7 задач GPT/Codex (plan-first ×3), 85 сценариев, 95 TC, 6 саботажей red→revert; protected src/server.js = ровно +7/-0 (2 mount-блока). Гейт 23 suites / 312 tests + FE build. **НЕ задеплоено** (owner-gated; деплой = код + FE-ребилд + миграция 172 + правка live-Caddyfile (мержить в существующий с /aihelper!) + DNS rate.albusto.com A→Vultr).
- OUT OF SCOPE (след. фазы): SMS-отправка + триггер job→Done, referrals/rewards, миграция ABC-Vercel-страницы на новый API.

## 2026-07-14 — RATE-ME-CRM-002: человечная conversion-страница Rate Me (персонализация из job) + review→job атрибуция + dispatcher «Send rating link»

**Фаза 2 RATE-ME-CRM-001 — чисто ADDITIVE UX+данные поверх 001-инфры. Ни один 001-контракт не тронут** (uniform-404 квинтет, replay-идемпотентность `technician_ratings.rate_token_id UNIQUE`, host-gate, `google_review_url`, rely-settings GET/PUT, public GET/POST-шейпы — байт-в-байт; новые поля аддитивны). Пайплайн GPT-implementer (Codex gpt-5.6-sol; Claude — архитектор/ревьюер): 8 задач T1–T8 + T9 финальный verify.

- **Публичная страница (полный rewrite `RatePage.tsx`):** голая 001-оценка превращена в человечный **7-экранный flow, персонализированный из job** — invitation → google-helper → happy → feedback → feedback-thanks → already-rated → expired (+ generic invalid). **GOLD-звёзды** (`#E0A72C`/`#D2D2D0`) — единственный не-violet акцент; ВСЕ прочие действия («Write my Google review», «Send to the team», «Book Visit», «Book your next visit →») = `var(--blanc-accent)`; строка «Blanc» не рендерится. **Prompt-чипы** (Punctuality·Clear explanation·Tidy work·Fair price·Friendliness / Timing·Communication·The repair·Pricing) — инертные `<button>`-подсказки, НИЧЕГО не вставляют в textarea. 5★ → 001 `POST /rating` → **click-beacon** (`keepalive`, fail-ignored) → `window.open('_blank')` в **НОВОЙ вкладке** (НИКОГДА `location.replace` — thank-you остаётся виден). Персонализация из токена (`job_id` → LEFT JOIN jobs/contacts): first-name (fallback customer_name→«Hi there,»), service label, visit date (Intl в tz компании, bad-tz→null), per-company контакты (`tel:`/`mailto:`) и booking-URL — **omit-when-null**. Rebooking-блок на happy/already-rated/expired; НЕТ на feedback-thanks (тон). **Expired-vs-invalid split (D-EXP):** узнанный host-binding connected просроченный токен → branded rebooking-payload (`expired:true`, хард-вайтлист 6 ключей); любой truly-invalid (unknown/malformed/foreign-host/disconnected) → 001 uniform-404 БЕЗ данных компании (non-oracle). PII-минимум: наружу уходит ТОЛЬКО `first_name` (LIVE = хард-вайтлист 12 ключей).
- **Атрибуция (миграция 178):** `rate_tokens` += `opened_at`/`google_click_at`/`sent_at`/`sent_via` (все nullable, `ADD COLUMN IF NOT EXISTS`, БЕЗ индекса, БЕЗ колонки booking_url). `opened_at` — стамп на первом GET (best-effort, live-only, first-open-only); `google_click_at` — на beacon; `sent_at`/`sent_via` — на dispatcher-send.
- **Click-beacon:** NEW `POST /api/public/rate/:token/click` (public, под уже-смонтированным `/api/public` + host-gate allowlist `/^\/api\/public\/rate(?:\/|-domain-ask)/` уже матчит — **НОЛЬ строк `src/server.js`**); идемпотентный first-click-wins стамп → `204` пустое тело; company/job ТОЛЬКО из токена; чужой/expired/unknown → uniform-404.
- **Job-card «Rate Me» блок + Send-rating-link (JOBS-поверхность):** NEW `POST /api/jobs/:id/rate-link` (`messages.send`) минтит СВЕЖИЙ токен и шлёт его по **SMS / Email / Copy** через канонический `conversationsService.sendMessage` / `emailService.sendEmail` (зеркало eta/notify: wallet-gate 402, NO_PHONE/NO_PROXY/NO_EMAIL 422, MAIL_DISCONNECTED 409, SMS_FAILED 502; стамп `sent_*` ТОЛЬКО при успехе канала — фейл не даёт false-«sent»). NEW `GET /api/jobs/:id/rate-status` (`jobs.view`) кормит attribution-таймлайн Job-карты (sent → opened → rated ★N → opened-google, каждый шаг только при наличии ts). **Рейтинг читается по `(company_id, job_id)`, НЕ по последнему токену** — re-send НИКОГДА не прячет существующую оценку. FE: `JobRateMeBlock.tsx` + FORM-CANON `RateLinkModal.tsx` (SMS/Email/Copy, disable без телефона/почты с честной причиной).
- **`booking_url` setting:** per-company rate-me-настройка (JSONB в `marketplace_installations.metadata.settings`, БЕЗ DB-колонки); `validateRateMeSettingsInput` парсит и **ВОЗВРАЩАЕТ ОБА** ключа (иначе replace-on-PUT вытер бы `google_review_url`); валидация зеркалит google (https, ≤500, иначе 400 `INVALID_BOOKING_URL`); audit — только booleans (URL-строки не попадают). FE `RateMeSettingsDialog` шлёт полный объект `{google_review_url, booking_url}`.
- **5 named sabotage-контролей red→restore (все проверены):** SAB-CONTEXT-PII-LEAK (12/6-key вайтлист + host-bind), SAB-GOOGLE-SAME-TAB (`window.open('_blank')` + beacon-стамп), SAB-BUBBLE-INSERTS-TEXT (чипы инертны), SAB-SENDLINK-CROSS-TENANT (company-scope job-load + queries), SAB-ATTRIBUTION-WRONG-JOB (target из токена, rating по `(company,job)`).
- Коммиты: af7965b (docs-chain) → 1560b64 (T1 mig178+rateMeQueries) · c34a533 (T2 rateMeService персонализация/expired/атрибуция) · 3148bde (T3 beacon+branded-expired GET) · e6a4591 (T4 booking_url setting) · f835792 (T5 jobs send-link+rate-status) · 218150b (T6 RatePage 7-экранов) · 297ec06 (T7 Job-card блок+modal) · 9064b30 (T8 settings-поле); T9 = final verify без кода. **Гейт: 6 rate-me-сьютов / 152 tests зелёные** + FE build + stay-green (jobs-пятёрка, marketplace/rely, mount-пины, tenant-lint). Protected НОЛЬ-diff: `src/server.js`, `frontend/src/lib/authedFetch.ts`, `frontend/src/hooks/useRealtimeEvents.ts`, `companies`-схема, `submitRating`, rely-тесты.
- **Закоммичено на ветке, НЕ задеплоено** (owner-gated; dark-safe — миграция аддитивна/идемпотентна, роуты/FE gated per-company install; без применения CRM байт-идентичен): psql-apply `179_rate_token_attribution.sql` (как 177, НЕ boot-registered) → FE rebuild → force-logout-all KC-сессий (stale SPA-чанки). RE-CHECK номер 178 vs origin/master перед push (parallel-migration-collision). Dispatcher-инициированный send закрывает часть 001-OUT-OF-SCOPE (SMS/email отправка ссылки); автоматический триггер job→Done и referrals/rewards остаются в след. фазах.

## 2026-07-15 — JOB-RECORD-PAYMENT-001: offline (cash/check) payment recording from the Job card + Pay-by-Card redesign

**Orchestration run (Codex gpt-5.6-sol implementer; Claude architect/reviewer + sabotage). Reuses the canonical `payment_transactions` ledger — NO new table, migration, or permission.**
- **Backend:** new `POST /api/jobs/:id/record-payment` (gate `payments.collect_offline`): tenant guard `jobsService.getJobById(id, companyId)` → 404 foreign; actor = `req.user?.crmUser?.id || null` (NEVER Keycloak sub — recorded_by → crm_users FK); validates `amount > 0` and `payment_method ∈ {cash, check}`; delegates to `paymentsService.recordManualPayment` (job-level row: job_id set, invoice_id null, external_source manual). `paymentsService.createTransaction` now honors a client `processed_at` (payment date), defaulting to now — persists the chosen date instead of always "now". `tests/jobRecordPayment.routes.test.js` (10 cases), sabotage-proven: SAB-CROSS-TENANT (neuter guard → TC-RP-2 red), SAB-RECORDEDBY-SUB (actor→sub → TC-RP-1/4 red). No regression: 135/135 payment+stripe suites.
- **Frontend:** «Collect payment» → «Pay by Card»; new «Record Payment» button in an **Option-A footer INSIDE the Estimated/Invoiced/Due card** (hairline `mt-px`, `grid grid-cols-2 gap-2` when both / `flex` full-width when one). Pay-by-Card gated `canCollect && stripeReady`; Record gated `payments.collect_offline` (no Stripe needed). New `JobRecordPaymentDialog` (right-side FORM-CANON panel: Amount prefill=Due · Payment method Cash/Check · Reference number · Payment date default-today · Internal note). `useJobFinancials` also fetches completed job-level ledger payments (fail-quiet); **Due now subtracts job-level payments with `invoice_id NULL`** (no double-count with invoice `amount_paid`), clamped ≥ 0 — also fixes the pre-existing gap where ad-hoc card payments never showed on the job. Payments list rendered on the card. `paymentsCanonicalApi.recordJobPayment` + `processed_at`. FE build (tsc -b) clean; footer + dialog visually verified via financials-harness.
- Commits: ad80b2c (backend) · 533a7d1 (frontend). Spec: `Docs/specs/JOB-RECORD-PAYMENT-001.md`. Owner decisions: visibility = reduce Due + card list · attribution = job-level. **NOT deployed** (owner-gated). Backend-only-safe: additive route + backward-compat service change; FE gated by the offline perm. Deploy = rsync → build → up -d app → logout-all → smoke (no migration).

## 2026-07-17 — NOTE-PHOTO-COMPRESS-001: клиентское сжатие фото в заметках + фикс вложений на лидах

**Тандем-режим (/tandem, первый прогон): Codex — discovery/предложение/весь код; Claude — развилки, гейты, sabotage.** Общий `NoteAttachmentInput` (jobs/leads/contacts) теперь сжимает фото НА КЛИЕНТЕ до отправки (нативный canvas, ноль зависимостей): длинная сторона ≤2560px, JPEG q0.8, скип ≤750KiB, EXIF-ориентация запекается, серийная обработка с yield (UI не мёрзнет), чипы Compressing…→Uploading…. Фикстуры владельца: 2.7MB→655KB / 5.3MB→629KB (поворот сохранён) / 2.7MB→597KB; MODEL/SERIAL на лейбле читаются (харнес `frontend/photo-compress-harness.html`, бинарники фикстур в .gitignore). HEIC-без-декода/GIF/WebP/не-картинки — passthrough (без регресса); лимит 10MB для картинок теперь ПОСЛЕ сжатия. Заодно починены вложения заметок на ЛИДАХ (фронт шлёт uuid, роут делал parseInt → 400): `resolveEntityIdInCompany` uuid→serial_id (конвенция note_attachments для лидов), tenant-scoped, foreign→404 + убран sub-fallback актёра. Тесты: vitest 6 (sabotage: обход сжатия → 5 red) + backend 12/12; FE build чистый. Коммит 3cb9be0. НЕ задеплоено (owner-gated).
## 2026-07-18 — STRIPE-PAYFORM-UX-001: минимальная форма списания карты + явный успех + знаковый Due

Мерчантская форма «Enter card manually» (Job/Invoice) пересобрана: вместо тяжёлого Payment Element (все методы аккаунта + email/телефон от Link) — композитный Stripe Card Element, ровно четыре поля **Card number / MM/YY / CVC / ZIP** (PAN/expiry/CVC/ZIP остаются в iframe Stripe). Backend: мерчантский PaymentIntent теперь `payment_method_types:['card']` (публичная страница оплаты и её automatic-methods путь НЕ тронуты — закреплено тестом-инвариантом), добавлен tenant-scoped read-only endpoint `GET /api/payments/manual-card-sessions/:id/result` → ровно `{status, amount, brand, last4}` (`payments.collect_keyed`, 401/403/чужая-компания-404). Успех = только `PaymentIntent.status==='succeeded'`: экран «Payment successful · $95.00 · Visa •••• 4242», закрывается только «Done»; сетевой разрыв после подтверждения реконсилируется по result-endpoint (0/1/3/7/15s) без возможности даблчарджа. Финансы: сводка Job стала **Estimated · Invoiced · Paid · Due**, Due — знаковый (кредит `−$95.00` U+2212, кредитный тон) на Finance-табе и мобильной плитке/списке (batch-роллап учитывает standalone-платежи без даблкаунта инвойсных). Тандем: спека+код Codex (gpt-5.6-sol xhigh), рамка/развилки/гейты Claude; макет утверждён владельцем до имплементации. Верификация: backend jest 102/102 (+саботаж публичного инварианта red→green), frontend build 0 + vitest 76/76 (+саботаж знакового Due red→green). Ручной QA со Stripe test-cards на стейдже — после деплоя.
...
## 2026-07-18 — DOC-SEND-NOTE-001: заметка на job о каждой отправке документа

Каждая отправка документа теперь оставляет след в заметках job: «Receipt for $95.00 sent to x@y.z», «Invoice #1042 sent to x@y.z», «Estimate #17 sent to x@y.z» (SMS-вариант: «… sent by SMS to +1…», номер в E.164). Покрыты все три пути: чек Stripe (STRIPE-RECEIPT-001), email/SMS-отправка инвойса и эстимейта. Автор заметки = реальный отправитель (`crmUser.id`, sub-фоллбэка нет — нет актора → заметка скипается с warn). Новый общий `documentSendNoteService` (одна точка копии + non-fatal обёртка): сбой заметки НИКОГДА не ломает отправку (warn без PII); заметка пишется после статусов/событий доставки. Документ без job-привязки → отправка ок, warn без получателя. Тандем: backend jest 171/171, саботаж копии red→green. Цель — одноразовые/«чужие» адреса остаются зафиксированными и переиспользуемыми.
...
## 2026-07-20 — PULSE-LEAD-PIN-001: закреплённая compact Lead bar в Pulse

In-flow Lead-карточка высотой 560px удалена из timeline: вместо неё Lead теперь
остаётся в общем с AR plaque `.pulse-sticky-stack` как owner-approved 68px bar, а
полный неизменённый `LeadDetailPanel` открывается в canonical right panel / mobile
bottom sheet без изменения scroll height. Identity: `LEAD #id · repair type`, name
и inline 26px status state-pill с dot/tint; actions: Edit, Convert, реальное overflow,
динамический Activate и expand. Status pill переиспользован и в full-panel header;
Source не переносился. Contact и Lead теперь делят shell/action/expand primitives,
один container-query ladder и shared Lead action/status renderers. Frontend-only,
без API/backend/schema изменений. Verification: production build exit 0; Vitest
43 files, 248/248; `SAB-LEAD-STICKY-MEMBERSHIP` дал 1 red / 11 green и после
восстановления вернулся к 12/12.

---

## 2026-07-20 — INSPECTOR-AGENT-001: marketplace-агент «Инспектор» (зависшие работы/лиды → задачи)
...
## 2026-07-21 — CHATGPT-CRM-MCP-001 S1: OAuth-коннектор ChatGPT к CRM (подключение + AI-личность + чтение)

Первый этап доступа ChatGPT к CRM через нативные MCP-коннекторы. **Только чтение** (18 инструментов: работы/лиды/контакты/расписание/задачи/сметы/инвойсы). Подключение — OAuth 2.1 через **Keycloak** (свой OAuth-сервер не строим): Albusto отдаёт RFC 9728 protected-resource metadata + сам `/mcp/chatgpt`, клиент `chatgpt-crm-mcp` регистрируется в Keycloak (PKCE, без секрета). Действует **выделенная AI-учётка-диспетчер** (kind=agent, без членства в компании) с deny-by-default набором прав ровно под выставленные инструменты — НЕ буквальная роль Диспетчера; `created_by`=AI-user. Fail-closed middleware проверяет aud/azp/resource/scope + цепочку binding (issuer,subject,client)→installation/company/AI-user с перепроверкой тенант-членства в одной company-scoped транзакции. Миграции 195/196; `src/server.js` — ровно 4 строки (2 mount + 2 require, разрешено владельцем). Тандем + attack-only red-team: **межтенантных утечек в 18 read-инструментах нет**; red-team поймал critical — connector-токен работал на обычных `/api/*` как админ → починено (`keycloakAuth` отвергает токены с azp/client_id = MCP-клиента до резолва человека). Верификация: 7 сьютов/110 тестов + auth-регрессия 46; sabotage red→restore. ⚠️ НЕ задеплоено; S2 (записи + CRUD смет/инвойсов) требует pre-write recheck на disconnect (в спеке). Оплаты (S4) отложены владельцем. Спека: `docs/specs/CHATGPT-CRM-MCP-001.md`.

---

## 2026-07-23 — ONBOARD-LOOP-FIX: backend reproduction of onboarding/super-admin auth loop

Backend real-component reproduction confirms no Root-B server defect: with
`FEATURE_SMS_2FA=true`, an active `tenant_admin`, active company, verified phone,
and zero trusted devices, `GET /api/auth/me` returns 200 **with company**; the
same Bearer identity receives `401 PHONE_VERIFICATION_REQUIRED` on a non-exempt
tenant route. The resolver runs before the 2FA gate, and `/api/auth/*` is
exempted before trusted-device enforcement. A platform `super_admin` correctly
resolves to platform scope with null company and receives 200 from `/api/auth/me`;
platform routes tolerate null company while tenant routes deliberately return
403. No backend production code change was needed. Added a rolled-back
real-PostgreSQL Jest harness (3 incident regressions; final auth run 30/30),
proved five named sabotage controls red→restore→green, and added a precise
frontend handoff: distinguish authz loading/success/failure, only onboard a
...
- OB-65 (tandem): appSandbox fixture contract realigned to the rewritten payments Job-projector — the App-Studio sandbox mock matcher now recognizes `WITH original_payments … ledger_effects …` (splitting the allocation query `FROM ordered` → `legacy_paid/capacity` from the ledger pool `FROM ledger_effects … GROUP BY` → `native_pool/total_pool`), and the phantom `postal_code` was dropped from sandbox jobs (`jobs` has no such column; kept on contacts). Pure test-drift — `jobsService.listJobs` untouched. `appSandboxFixtureContract` 1/1; sabotage (break matcher → red, "Unexpected Job projector SQL") → restored.
