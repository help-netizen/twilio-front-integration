...
- **Закоммичено на ветке, НЕ задеплоено** (owner-gated; dark-safe — миграция аддитивна/идемпотентна, роуты/FE gated per-company install; без применения CRM байт-идентичен): psql-apply `178_rate_token_attribution.sql` (как 177, НЕ boot-registered) → FE rebuild → force-logout-all KC-сессий (stale SPA-чанки). RE-CHECK номер 178 vs origin/master перед push (parallel-migration-collision). Dispatcher-инициированный send закрывает часть 001-OUT-OF-SCOPE (SMS/email отправка ссылки); автоматический триггер job→Done и referrals/rewards остаются в след. фазах.
