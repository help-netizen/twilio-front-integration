...
- **Фикс** (`ScheduleJobsMap.tsx`): plottable-гейт теперь **только по наличию координат** — `jobs.filter(j => j.lat != null && j.lng != null)`, ровно как в десктопном `CustomTimeModal` (он никогда не смотрит `geocoding_status`, рисует любую работу с truthy lat/lng). Счётчик «N без адреса» теперь считает только работы без координат. Спека (Plottable set, S4) обновлена.
- **Проверка:** прод-данные подтвердили диагноз (4/4 работы Robert: `geocoding_status='not_geocoded'`, но `lat/lng` заполнены); `npm run build` зелёный.
