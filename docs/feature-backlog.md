...
### `P3` Переименовать внутреннюю дизайн-систему/неймспейс Blanc → Albusto (BLANC-RENAME-001)

**Контекст.** Продукт переименован в **Albusto**, но внутреннее имя дизайн-системы и неймспейса осталось **Blanc** (переименовывали только продукт + отдельно палитра Т2/PALETTE-V2 — это про цвета, не про имя). Правило сейчас: `--blanc-*` — внутреннее, слово «Blanc» в UI-тексте не шипим; UI-утечек нет (проверено 2026-07-07 — все совпадения = внутренние имена, пользователю не видны). Так что это **не баг, а косметический тех-долг** ради консистентности.

**Масштаб (что переименовать):**
- CSS-токены `--blanc-*` — **~198 файлов** (`.css`/`.tsx`/`.ts`), плюс определения токенов в `theme.css`/`index.css`.
- CSS-анимации `blancSlideUp` / `blancFadeIn` / `blancSlideInRight` (`BottomSheet.tsx`, `dialog.tsx`, `Overlay.tsx` + keyframes).
- Код-константы `BLANC_STATUSES` / `BLANC_STATUS_COLORS` / `BLANC_NS` (workflows/jobs).
- **⚠️ РИСК: колонка БД `blanc_status`** — реальная колонка `jobs.blanc_status`, задета в множестве SQL (backend queries), а также в мобильном `albusto-mobile` (`job.blanc_status`) и FSM. Переименование колонки = миграция + синхронная правка backend + web + mobile + голосовых скилов → самая дорогая и рискованная часть; можно вынести в отдельный под-таск или оставить `blanc_status` как есть (историческое имя, невидимо), переименовав только фронтовые токены/константы.
- CLAUDE.md `### Design System (Blanc)` + упоминания в `Docs/*`.

**Подход.** Механический codemod по токенам/константам/анимациям (низкий риск, чисто фронт), колонку БД — отдельным решением (высокий риск/низкая ценность, возможно НЕ трогать). Гейт: `npm run build` + backend jest. Кандидат на /orchestrate отдельным прогоном. НЕ блокирует текущие фичи — новый код продолжает использовать существующие `--blanc-*` токены до рефактора.
