...
## STRIPE-ADHOC-PAY-001: Ad-hoc Stripe payment collection with arbitrary amount from the Job card (Finance tab)

**Status:** Requirements · **Priority:** P1 · **Date:** 2026-07-07
**Builds on:** F018 (Stripe Payments Marketplace, Phases 1–5). Adds an
**invoice-INDEPENDENT** collect flow surfaced directly on the Job → Finance tab.

**Краткое описание:** Office user with `payments.collect_*` may charge an **arbitrary,
editable amount** to the customer directly from the Job card's Finance tab — no invoice
required. Three methods: **enter card manually** (Stripe Payment Element), **send a
payment link** (email + SMS to the job's contact), **copy a payment link**. The payment
is recorded as a **standalone job payment** (`payment_transactions.job_id`, NO
auto-created invoice) on the canonical ledger. When Stripe is not `connected_ready`,
the button's place shows a promo/CTA card instead.

**Связь с существующими фичами (НЕ дублирует):**
- **Расширяет F018.** Today the only web collect UI is invoice-only, buried in
  `InvoiceDetailPanel.tsx` (Collect-payment dropdown + `ManualCardDialog` bound to
  `invoiceId`), so an owner with no open invoice has no entry point. This adds a
  **Finance-tab-level** entry point on the job.
- **Reuses the F018 ledger + backend manual-card-on-job path** (arbitrary amount on a job
  already works server-side: `POST /api/jobs/:id/stripe-manual-card-session` →
  `createManualCardSession({jobId, amount})` → `resolveSurfaceContext({jobId, amount})`);
  the manual-card method needs **no backend work**.
- **Backend gap:** payment **link** creation (`ensurePaymentLink`) is invoice-only.
  One backend task generalizes Checkout-session/link creation to `{jobId, amount}`.
- **Does NOT touch the invoice collect flow** — invoice Collect/Send/Copy, public pay
  page, and platform billing stay byte-for-byte unchanged.

**Продуктовые решения владельца (зафиксированы):**
- **D1:** amount prefill = job outstanding (`totalInvoiced − totalPaid`, if > 0, else
  blank), fully editable.
- **D2:** "send link" = email **and** SMS to the job's contact, plus a "copy link" button.
- **D3:** ad-hoc payment is standalone on the job (`job_id`, **NO** auto-created invoice).
- **D4:** when Stripe ≠ `connected_ready`, the button's place shows an English promo/CTA
  card (copy per state + per permission — see FR-CTA).
- Button/CTA lives in `JobFinancialsTab` near the Estimate/Invoice blocks.
- All UI in English; Blanc `--blanc-*` tokens; CTA card on `--blanc-surface-muted`.

**Пользовательские сценарии (use cases):**
1. **UC-1 (happy path — manual card).** Office user (`payments.collect_keyed` or
   `payments.collect_online`) opens Job → Finance, Stripe is `connected_ready`, sees a
   **Collect payment** button, clicks it → dialog prefilled with the outstanding amount
   (editable) → picks **Enter card manually** → Payment Element → charge succeeds →
   standalone job payment appears on the ledger/timeline.
2. **UC-2 (send link).** Same, picks **Send payment link** → link minted for the entered
   amount → email + SMS dispatched to the job's contact → confirmation.
3. **UC-3 (copy link).** Same, picks **Copy payment link** → link copied to clipboard.
4. **UC-4 (Stripe not ready).** Stripe is `not_connected` /
   `onboarding_incomplete` / `action_required` → the button's place shows a CTA card
   with state-appropriate copy and a [Connect Stripe] / [Finish setup] button routing to
   Settings → Integrations → Stripe Payments.
5. **UC-5 (no integrations permission).** Same as UC-4 but the user lacks
   `tenant.integrations.manage` → CTA shows "Ask an account admin to connect Stripe in
   Settings → Integrations." with **no** action button.
6. **UC-6 (provider edge).** A provider on their own job with a `payments.collect_*`
   grant sees the button under the same gate; a provider without any collect grant sees
   neither the button nor the CTA promo (no dead affordance).

**Функциональные требования:**

_FR-BTN — Collect-payment button + gating (frontend)_
- **FR-BTN-1.** In `JobFinancialsTab`, near the Estimate/Invoice blocks, render a
  **Collect payment** button **iff** Stripe status = `connected_ready` **AND** the user
  has any of `payments.collect_online` / `payments.collect_offline` /
  `payments.collect_keyed` on this job.
- **FR-BTN-2.** If the user has **no** collect permission at all → render nothing (no
  button, no CTA).

_FR-CTA — placeholder / promo states (frontend)_
- **FR-CTA-1.** When the user **has** a collect permission but Stripe ≠ `connected_ready`,
  render an English CTA card **in the button's place** on `--blanc-surface-muted`
  (clean, per CLAUDE.md — no `<hr>`, no empty rows).
- **FR-CTA-2.** State copy:
  - `not_connected` → title "Accept payments right from the job", body "Connect Stripe to
    charge your customer's card or send a payment link in seconds — no invoice required.",
    button **[Connect Stripe]**.
  - `onboarding_incomplete` / `action_required` → "Finish your Stripe setup to start
    collecting payments" + button **[Finish setup]**.
  - Both buttons route to **Settings → Integrations → Stripe Payments**.
- **FR-CTA-3.** If the user lacks `tenant.integrations.manage`, replace the button with
  the text "Ask an account admin to connect Stripe in Settings → Integrations."

_FR-DLG — Collect-payment dialog (frontend)_
- **FR-DLG-1.** `CollectPaymentDialog` follows FORM-CANON (right-side panel /
  bottom-sheet on mobile).
- **FR-DLG-2.** Step 1 = amount, prefilled per **D1** (job outstanding or blank), editable,
  validated **> 0 and ≤ sane max** (see constraints).
- **FR-DLG-3.** Step 2 = method chooser: **Enter card manually** (FR-CARD) / **Send
  payment link** (FR-LINK) / **Copy payment link** (FR-LINK).

_FR-CARD — manual card via existing job route (frontend only)_
- **FR-CARD-1.** Generalize `ManualCardDialog.tsx` from `invoiceId`-only to
  `{ jobId?, invoiceId?, amount }`; for the job path call the **existing**
  `POST /api/jobs/:id/stripe-manual-card-session` with the entered amount.
- **FR-CARD-2.** No backend change for manual card.

_FR-LINK — ad-hoc job payment link + send + copy (1 backend task + frontend)_
- **FR-LINK-1.** **NEW** backend: generalize Checkout-session/link creation to accept
  `{ jobId, amount }` (invoice-independent), reusing the existing
  `stripe_payment_sessions.job_id` column (no migration expected).
- **FR-LINK-2.** New routes: `POST /api/jobs/:id/stripe-payment-link` (create/reuse),
  `GET` active + history, `POST /api/jobs/:id/send-payment-link`. Gated
  `payments.collect_online`; `company_id` from `req.companyFilter.company_id`; all SQL
  company-scoped.
- **FR-LINK-3.** Link creation is idempotent, mirroring the invoice path (reuse a valid
  open session rather than minting duplicates).
- **FR-LINK-4.** **Send** dispatches to the **job's contact** via the shared messaging
  path — email **and** SMS (**D2**). **Copy** returns the link URL for the clipboard.

_FR-LEDGER — standalone job payment (backend, mostly existing)_
- **FR-LEDGER-1.** A successful ad-hoc charge writes **one** row to canonical
  `payment_transactions` with `job_id` set and **no invoice** (via
  `paymentsService.createTransaction`; `resolveSurfaceContext` already supports `jobId`).
- **FR-LEDGER-2.** Idempotency mirrors the invoice path (webhook by `stripe_event_id`;
  ledger by `(company_id, external_id)`).
- **FR-LEDGER-3.** No auto-created invoice (**D3**).

**Ограничения и нефункциональные требования:**
- **Backward-compat:** the invoice collect / send / copy flow, public pay page, and
  platform billing are **unchanged**.
- **Scope:** frontend + **exactly one** backend task (FR-LINK job payment-link). Manual
  card + ledger reuse existing F018 backend.
- **Company-scope:** all new API `authenticate, requireCompanyAccess`; `company_id` only
  from `req.companyFilter.company_id`; every SQL filters by `company_id`.
- **Amount validation:** `> 0` and `≤` a sane max (architect to fix the ceiling; align
  with any existing invoice/manual-card max).
- **PCI:** card data only via Stripe-controlled UI/SDK; Albusto never stores/logs PAN/CVC.
- **No migration expected** (`stripe_payment_sessions.job_id` already exists).
- **Design:** English only; Blanc tokens; CTA on `--blanc-surface-muted`; no `<hr>`, no
  empty rows (CLAUDE.md).

**Потенциально вовлечённые модули/части системы:**
- Backend (1 task): `backend/src/services/stripePaymentsService.js` (generalize
  `ensurePaymentLink` / `sendPaymentLink` from `invoiceId`-only to `{ jobId, amount }`;
  resolve the job's `contact_id` in the `jobId` branch of `resolveSurfaceContext` — today
  it leaves `contactId=null`); `backend/src/routes/jobs.js` (new payment-link routes
  alongside the existing `stripe-manual-card-session` @877 and `tap-to-pay` @885).
  Reuse: `paymentsService.createTransaction`, shared messaging path.
- Frontend: `JobFinancialsTab.tsx` (button + CTA, `totalDue` ~line 103),
  `ManualCardDialog.tsx` (generalize props to `{ jobId?, invoiceId?, amount }`), new
  `CollectPaymentDialog.tsx`, job payments API client, `utils/loadStripe.ts`,
  permission/status reads from `AuthProvider`.

**Затронутые интеграции:** Stripe (Checkout Sessions, Payment Element, webhooks). Twilio
(SMS send-link via shared messaging path) + email path. Not Front/Zenbooker.

**Защищённые части кода (НЕЛЬЗЯ ломать):**
- Invoice collect/send/copy path: `InvoiceDetailPanel.tsx`, `ensurePaymentLink` invoice
  behavior, public pay page — must stay functionally unchanged.
- F018 webhook + canonical ledger idempotency.
- Platform billing (`billingService`, `/api/billing`, `stripeProvider`,
  `/api/billing/webhook`) — untouched.
- `src/server.js` core middleware/SSE (mount-only if anything).

**Acceptance criteria:**
- **AC-1.** Button shows on Job → Finance **iff** `connected_ready` **AND** user has a
  `payments.collect_*` grant; hidden entirely with no collect grant.
- **AC-2.** CTA shows in the button's place for every non-`connected_ready` state with the
  exact copy of FR-CTA-2, and the no-`integrations.manage` variant (FR-CTA-3) shows the
  admin-ask text with no button.
- **AC-3.** An arbitrary amount charged via **manual card** records a standalone job
  payment (`job_id`, no invoice) on the ledger + timeline.
- **AC-4.** A payment **link** for an arbitrary amount can be **created, sent (email +
  SMS to the job's contact), and copied**; re-request reuses the open session.
- **AC-5.** The invoice collect/send/copy flow and public pay page are unchanged.
- **AC-6.** `npm run build` green (tsc -b + vite, prod-strict); backend `jest` green
  (incl. `tests/stripePayments.test.js`, 26 baseline + new link-on-job cases).

### Out of scope
- Refunds/disputes on ad-hoc job payments (reuse F018 Phase 5 when needed); Tap to Pay
  UI (no mobile shell); auto-creating an invoice from an ad-hoc payment; partial-amount
  reconciliation against a later invoice.

## Фича YELP-LEAD-AUTORESPONDER-001 (Phase 1a — email-only, backend)

**Краткое описание:** When a Yelp *new-lead* email is ingested (forwarded to
help@bostonmasters.com → Gmail sync, real-time after GMAIL-PUSH-FIX-001), auto-send **one**
LLM-personalized greeting back through the Yelp email relay **and** create an Albusto lead —
so the customer is answered in seconds and a lead surfaces for the dispatcher, 24/7. Backend-
only, additive to the Mail Secretary; non-Yelp email is untouched.

**Пользовательские сценарии:**
1. Customer submits "Request a quote" on Yelp → new-lead email lands → within seconds the
   customer receives a warm greeting referencing their appliance/problem and asking for best
   phone + time; a new "Yelp" lead appears in Pulse.
2. Customer *replies* on the same Yelp thread, or Yelp sends a no-reply confirmation → system
   does **not** greet again (no duplicate greeting, no duplicate lead).

**Функциональные требования:**
- **R1 — Trigger & scope.** Runs inside the existing Gmail ingest seam
  (`emailSyncService.importGmailThread`) for inbound `email_messages`; only for the
  company/mailbox receiving the Yelp forward; `company_id`-scoped end-to-end.
- **R2 — New-lead detection (must ALL hold).** `from_email` matches
  `reply+<hextoken>@messaging.yelp.com`; body/links carry `utm_source=request_a_quote_first_message`;
  body header matches `"<Name> requested a quote from <Biz> for a <service>."` plus a labeled
  Q&A block. This is the ONLY class that triggers a greeting.
- **R3 — Negative detection (never greet).** Customer replies (`utm_source=request_a_quote_new_message`,
  subjects like "…response to <Biz>" / "New Message from <Name>") and Yelp confirmations
  (`no-reply@yelp.com`, `no-reply@notify.yelp.com`, "Good news! Your request was sent.") are
  ignored — no greeting, no lead.
- **R4 — Parse.** Extract `{ name, service, problem, zip, reply_to, thread_token, magic_link }`
  from the header + Q&A (problem = brand+appliance+symptom, e.g. "Maytag dishwasher stuck in
  mid cycle"; zip from the location answer; `reply_to` = the `From reply+…` address;
  `thread_token` = the hextoken).
- **R5 — Greeting (exactly one).** Send **one** Gemini-generated personalized reply (reference
  the appliance/problem, ask best phone + preferred time, **NO price**) via
  `emailService.sendEmail(companyId, { to: reply_to, … })` so Yelp relays it to the customer.
- **R6 — One-greeting-per-thread.** Yelp accepts only one email reply per thread → the system
  must guarantee at most one greeting per `thread_token` (guard before send).
- **R7 — Create lead.** Create an Albusto lead via the existing `leadsService.createLead`
  (name / service / problem / zip, `source='Yelp'`); it surfaces in Pulse like any new lead.
- **R8 — Idempotency (one greet + one lead per thread).** Dedupe on `thread_token` /
  `provider_thread_id`: skip greeting if an outbound `email_messages` row already exists on the
  thread; skip lead if one already carries the token. Store a marker in `leads.metadata`
  (`{ yelp_thread_token, greeted_at, greeting_provider_message_id }`) — **no migration** (leads.metadata JSONB exists).
- **R9 — Safe-fail.** Parse / Gemini / send failures MUST NOT crash ingest and MUST NOT block
  other mail; errors are logged; the lead is still created even if the greeting fails.

**Нефункциональные требования:**
- **N1.** Backend-only, additive to the Mail Secretary pipeline (`agentWorker` /
  `rulesEngine` / `ruleActions`); non-Yelp email behavior unchanged.
- **N2.** All reads/writes `company_id`-scoped; greeting only via the company's own mailbox.
- **N3 — Latency (success criterion).** Greeting delivered **within seconds** of ingest
  (bounded by Gmail push + Gemini round-trip), 24/7 unattended.
- **N4.** Prefer no migration (reuse `leads.metadata` + existing `email_messages` rows for
  idempotency); Gemini via the existing provider seam, degrade safely on quota (cf.
  gemini-spend-cap outage). No new external service.

**Потенциально вовлечённые модули/части системы:**
- `backend/src/services/emailSyncService.js` (ingest hook @ `importGmailThread`),
  `emailService.js` (`sendEmail` relay-reply), `leadsService.js` (`createLead`, source='Yelp'),
  Gemini/LLM provider seam, `email_messages` / `leads` tables.

**Затронутые интеграции:** Yelp (inbound email relay — reply back through `reply+…@messaging.yelp.com`),
Gmail (existing sync/push), Gemini (greeting generation). Not Twilio/Stripe/Zenbooker/Front.

**Защищённые части кода (НЕЛЬЗЯ ломать):**
- Mail Secretary triage + `email_messages` ingest/dedup (`provider_message_id`) — Yelp path is
  purely additive; all other inbound mail behaves exactly as before.
- Existing lead-creation path / Pulse new-lead surfacing (reuse, don't fork).

**Acceptance / success criteria:**
- **AC-1.** A real Yelp new-lead email → customer receives exactly one greeting within seconds
  **and** a `source='Yelp'` lead appears in Pulse with {name, service, problem, zip}.
- **AC-2.** Customer replies and no-reply confirmations produce **no** greeting and **no** new
  lead (idempotent per thread).
- **AC-3.** Forced parse/LLM/send failure does not crash ingest and still yields the lead.

### Out of scope (YELP-LEAD-AUTORESPONDER-001)
- **Phase 1b (phone):** obtaining the customer phone number (requires Yelp login / browser
  automation) and any phone/SMS/voice follow-up — explicitly deferred.
- No browser or Yelp-login automation, no DNS/GCP changes, no price quoting in the greeting,
  no multi-message conversational back-and-forth (Yelp allows one email reply), no frontend/UI
  work beyond the lead appearing via the existing Pulse path.
