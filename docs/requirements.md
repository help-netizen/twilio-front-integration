...
- **NFR-RM2-11 (build + protected files).** FE build mandatory (`cd frontend && npm run build`). Protected/untouched: `src/server.js` (only a minimal flagged mount if a new public route is unavoidable, +N/-0), `frontend/src/lib/authedFetch.ts`, `frontend/src/hooks/useRealtimeEvents.ts`. `backend/db/` changes only via migration 178.
