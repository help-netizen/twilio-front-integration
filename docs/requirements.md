...
## RATE-ME-CRM-001 — Rate Me becomes a multi-tenant CRM marketplace app: per-tenant rating storage + public technician-rating page on rate.albusto.com or the tenant's own domain (CNAME), infrastructure phase (2026-07-13)

> Status: requirements (Product 01). NEW feature — no RATE-ME entry exists anywhere in requirements/code (grep-verified: zero `rate-me`/`rate_me` hits in backend/frontend/src/migrations). This is the **infrastructure phase only**: data model, public token API, public page, dedicated + custom-domain hosting, marketplace app + settings. **SMS sending, job-Done triggers, referrals, rewards, and any ratings-VIEWING UI are OUT OF SCOPE (later phases).** Binding owner decisions from the interview are marked **[OWNER]**: (1) option-A host = DEDICATED `rate.albusto.com`, which is ALSO the CNAME target for option B; (2) custom-domain verification = CNAME-only — one DNS record, we check resolution; (3) happy threshold = **5★ ONLY** → redirect to the company's Google-review link, 1–4★ → internal feedback textarea → thank-you (threshold NOT configurable this phase); (4) ratings are STORED only (no viewing UI this phase).

**Краткое описание.** The Rate Me rating engine (today a single-tenant page in the legacy lead-engine stack) becomes part of the multi-tenant CRM as a marketplace app (`rate-me`). Every tenant gets its OWN isolated rating system: opaque crypto tokens tied to (company, job, technician), a mobile-first branded public rating page, and per-company rating storage. The public page is hosted (A) on a new dedicated `rate.albusto.com` or (B) on the tenant's OWN domain, connected through a humane DNS form in the Rate Me app settings (one CNAME record pointing at `rate.albusto.com`; Caddy issues the certificate on demand via an ask-endpoint that authorizes ONLY verified domains). A 5★ rating immediately redirects the customer to the company's Google-review link (a new Rate Me setting); 1–4★ opens an internal feedback textarea and ends on a thank-you. Ratings and feedback are stored per company and are NOT surfaced anywhere in the CRM yet.

### Verified code/schema facts (downstream agents: do NOT re-derive, do NOT contradict)

- **Migration numbering:** latest file = `171_timeline_revpage_call_page_index.sql` → **next free = 172**. ⚠️ Header comments LIE about numbers after parallel-branch renumbering (170's header says "Migration 169", 123's header says "Migration 119") — the FILENAME is authoritative.
- **Marketplace settings scaffold (RELY-LEADS-SETTINGS-001, implemented, tests green):** `GET/PUT /api/marketplace/apps/:appKey/settings` (`backend/src/routes/marketplace.js:50-75`) under the mount `authenticate + requirePermission('tenant.integrations.manage') + requireCompanyAccess` (`src/server.js:268`, protected file — new settings behavior goes INSIDE marketplace.js). Whitelist = `SETTINGS_ENABLED_APP_KEYS = new Set(['rely-leads'])` (`backend/src/services/marketplaceService.js:36`); `resolveSettingsInstallation` = whitelist → published app → **connected installation, else 404**. ⚠️ Validation and response are currently **Rely-HARDCODED** (`validateRelySettingsInput`, `buildSettingsResponse` unconditionally return rely settings/catalogs/territory) → adding `rate-me` REQUIRES a per-app-key dispatch refactor with rely behavior byte-identical (suites `tests/relyLeadsSettings*.js` must stay green). Storage: `marketplaceQueries.setInstallationSettings` = `metadata || jsonb_build_object('settings', $3)` — top-level MERGE (seeded keys survive) but **`metadata.settings` is REPLACED wholesale** → PUT must send the full settings object. Audit precedent: `marketplaceQueries.writeEvent(eventType: 'settings_updated')`.
...
- **Public token-page precedent (`/e/:token`, SEND-DOC-001):** the page is a **route inside the MAIN SPA build** (`frontend/src/App.tsx:112` → `PublicEstimateViewPage`), NOT a separate build. Keycloak is bypassed via `PUBLIC_AUTH_PATHS = ['/signup', '/pay', '/e']` (`frontend/src/auth/AuthProvider.tsx:192`). Backend = unauthenticated router `backend/src/routes/public-estimates.js` mounted at `/api/public` in `src/server.js:239-246` BEFORE authed routers; token guard `TOKEN_RE = /^[A-Za-z0-9_-]{6,64}$/` + uniform 404 `Invalid link`. SPA + API are ONE Express on `:3000` (`express.static` + `index.html` catch-all, `src/server.js:355-368`, `PORT=3000`, listens 0.0.0.0); Caddy proxies `app.albusto.com, api.albusto.com → 127.0.0.1:3000`. Public DTO minimalism precedent: `getPublicEstimate` returns company_name + doc fields ONLY, no ids.
- ⚠️ **Estimate tokens are only 64-bit** (`crypto.randomBytes(8).toString('base64url')`, `estimatesService.js:695`) — rate tokens must NOT copy this; ≥128 bit required (NFR-1).
- **Technicians:** there is NO technicians table. Technician identity = **Zenbooker TEXT id**. Truth = `jobs.assigned_techs` JSONB (`t->>'id'`, `t->>'name'`); display name = `COALESCE(technician_profiles.name, assigned_techs name)` via `technicianProfilesService` (`backend/src/services/technicianProfilesService.js`; table `technician_profiles`, mig 123: `company_id UUID + tech_id TEXT UNIQUE(company_id,tech_id)`, `name` override, `photo_storage_key`) — built FOR the public payment page (`/pay/:token`), i.e. the exact precedent to REUSE for the rating page's display name. `technician_base_locations` (mig 125) and `technician_time_off` (mig 167) use the same `(company_id, tech_id TEXT)` key.
- **Company branding:** `companies` table — `name`, `logo_storage_key` (mig 134); `companyProfileService.getProfile` → `logo_url` = **presigned S3 URL** (`storageService.getPresignedUrl`, default TTL 1h), best-effort `null` on presign failure (a broken logo must not break the page).
- **Edge/Caddy:** `infra/Caddyfile` = in-repo REFERENCE copy; live `/etc/caddy/Caddyfile` is authoritative; apply/rollback = MANUAL procedure per `infra/README.md` (changelog 2026-07: "Infra now tracked"). TODAY the Caddyfile has **NO `rate.albusto.com` block and NO `on_demand_tls` anywhere** — both are NEW in this feature. Existing blocks: albusto.com static, app/api → :3000 (with `handle_path /apps/leads* → :4001` precedent), auth → :8081.
- **Public rate-limit precedent:** `express-rate-limit` per-IP in `backend/src/routes/publicAuth.js` (`limiter(max)`, 20–120/min per endpoint).

### Пользовательские сценарии (use cases)

- **US-1 (happy path, option A, Google link configured).** Customer opens the tokenized link on `rate.albusto.com` → mobile page with the company's logo + name and the technician's display name → taps **5★** → the rating is recorded server-side, then the browser immediately redirects to the company's Google-review link.
- **US-2 (unhappy path, internal feedback).** Customer taps **3★** → an internal feedback textarea appears ("What could we have done better?") → submits (text optional) → thank-you screen. Stars + feedback are stored under the company; NO Google redirect, nothing posted publicly.
- **US-3 (one-time / replay semantics).** Customer re-opens the same link after rating: GET context says `already_rated` → the page shows the thank-you state, no star picker. A direct second POST with the same token returns **200 already-recorded** and the stored rating is UNCHANGED (no overwrite, no error).
- **US-4 (custom domain, option B).** Owner opens Rate Me settings → "Your own domain" → types `rate.bostonmasters.com` → the form shows the EXACT record to create: `Type: CNAME · Host/Name: rate · Target: rate.albusto.com` → owner adds it at the registrar → presses **Verify** → status pending→verified; on the first customer visit Caddy asks our ask-endpoint, gets 200, obtains the certificate → status active, "Your rating page is live at https://rate.bostonmasters.com". The page renders THEIR branding on THEIR domain.
- **US-5 (tenant isolation probe).** A token minted for company B is opened on company A's custom domain → uniform 404. Guessing tokens on `rate.albusto.com` → uniform 404 + per-IP rate limit. Asking Caddy's ask-endpoint about a foreign/unknown domain → silent 404, no certificate ever issued.
- **US-6 (5★ but no Google link configured).** Rating is stored; the page falls back to the thank-you screen (no dead end, no error, no redirect).
- **US-7 (verification failure, humane).** Owner enters a domain but the CNAME doesn't exist yet / points elsewhere / owner typed an apex domain → Verify shows humane copy ("We can't see the CNAME record yet — DNS changes can take up to an hour. Check the record and try again." / "Use a subdomain like rate.your-domain.com — root domains can't carry a CNAME record") with a retry button; the domain is NEVER served (and never gets a certificate) until verified.
- **US-8 (ABC-Homes-later-migration note).** ABC Homes' existing rating page (their Vercel-hosted site on `abchomes-appliance.com`) keeps working, untouched — this phase changes nothing outside the CRM. When ABC Homes later becomes a tenant, option B needs only ONE NEW subdomain CNAME (`rate.abchomes-appliance.com` → `rate.albusto.com`); apex/www stay on Vercel, and the old page can keep running in parallel during the transition.
...
- **FR-3 (token mint seam — internal only).** A company-scoped service function `mint(companyId, jobId, techId)` creates a `rate_tokens` row and returns the public URL. **NOT wired to any trigger this phase** (job-Done/SMS = later phases). A permission-gated authed mint endpoint for manual/smoke use MAY be added (Architect) — no UI for it.
- **FR-4 (public GET context by token).** Unauthenticated, rate-limited, token-format pre-check (base64url guard) before any DB read. Returns ONLY: technician display name (COALESCE `technician_profiles.name` → `assigned_techs` name; presentation e.g. first name — page/Architect), company `name`, company `logo_url` (presigned, nullable), `already_rated` boolean, and behavior flags for the page (e.g. `five_star_redirect: true/false` = Google link configured). **NOTHING else leaks:** no company/job/tech/serial ids, no address/phone/status, no Google URL in GET. Unknown / malformed / expired / foreign-host / disconnected-app token → uniform 404 (`Invalid link` model of public-estimates).
- **FR-5 (public POST rating).** Body = `{stars: int 1..5 required, feedback: string optional (trimmed, length-capped ~2000)}`. **Company/job/technician are derived ONLY from the token server-side — NEVER from the body** (extra body fields ignored). First successful POST inserts `technician_ratings` + stamps `rate_tokens.used_at` (atomically; the `rate_token_id UNIQUE` constraint is the race-proof guarantee). Response carries the next step: `{recorded: true, next: 'google_redirect', redirect_url}` (only when `stars=5` AND the link is configured) or `{recorded: true, next: 'thanks'}`. **Replay: second POST for the same token → 200 `{recorded: false, already_recorded: true, next: 'thanks'}` — no overwrite, no error** [OWNER-confirmed semantics]. The redirect URL appears ONLY in the POST response (after recording) — guarantees record-before-redirect.
- **FR-6 (happy-threshold semantics) [OWNER].** Happy = `stars === 5` ONLY, NOT configurable this phase. 5★ → redirect to the company's Google-review link; link missing → thank-you fallback (rating still stored). 1–4★ → feedback textarea → thank-you. The Google-review link is a NEW `rate-me` app setting (FR-9), validated as an absolute `https://` URL.
- **FR-7 (public page).** Mobile-first, branded (company logo + name, technician display name, star picker), served on the rating hosts; path `/r/:token` or `/:token` — **Architect decides**. Interaction: tap 5★ → immediate POST → redirect; tap 1–4★ → textarea + submit (skippable); `already_rated` → thank-you view. Humane copy; NO "Blanc" string; NO CRM navigation/chrome. Serving mechanism (host-gated route in the main SPA — the `/e/:token` precedent, incl. a `PUBLIC_AUTH_PATHS` entry — vs a separate minimal page bundle) = Architect, but NFR-5 (CRM not reachable on rating hosts) is absolute either way.
- **FR-8 (Host-header binding).** The request Host determines tenant scope: on `rate.albusto.com` (option A, shared host) a valid token of ANY tenant resolves; on a custom domain the host MUST match a `verified`/`active` `rate_me_domains` row AND `token.company_id === domain.company_id`, else uniform 404. Unknown host on the rating surface → 404. Exact-host match only (no wildcards; `www.` not auto-included).
- **FR-9 (settings extension on the RELY scaffold).** Add `'rate-me'` to `SETTINGS_ENABLED_APP_KEYS` + introduce a **per-app-key settings dispatch** (validation + response shape per app) keeping `rely-leads` GET/PUT byte-identical. `rate-me` settings stored in `marketplace_installations.metadata.settings`: `{google_review_url: string|null}` (https-validated). **Domain lifecycle state lives in `rate_me_domains`, NOT in metadata** (the ask-endpoint queries by domain); the settings GET response MAY embed the current domain row (single panel payload) — Architect. PUT replaces `metadata.settings` wholesale (scaffold semantics) — FE always sends the full object.
- **FR-10 (custom-domain flow endpoints, authed).** Under the marketplace mount permissions (`tenant.integrations.manage`): set/replace domain (normalize lowercase/trim/punycode; reject invalid hostnames; **reject apex domains with humane copy suggesting `rate.<their-domain>`** — CNAME on apex is not generally possible [PRODUCT]); read state; **Verify** = server-side DNS CNAME resolution of the domain, target equals `rate.albusto.com` → `verified` (else `failed` + humane `last_error`); re-verify allowed anytime; remove domain (row deleted/disabled → ask-endpoint stops authorizing; certificate lapses at renewal). Setting a NEW domain replaces the old binding (UNIQUE(company_id)); a domain already claimed by ANOTHER company → validation error WITHOUT revealing who holds it ("This domain is already in use").
- **FR-11 (activation).** `verified → active` on the first positive ask-endpoint authorization for that domain (or first successful page GET on that host — Architect picks the signal and documents it); UI then shows "Your rating page is live at https://<domain>". `active` and `verified` are BOTH serve-authorized; the split exists purely for humane status copy.
- **FR-12 (Caddy fragment, in-repo + manual apply).** Extend `infra/Caddyfile` (reference) + `infra/README.md` (procedure): (a) NEW `rate.albusto.com` site block → `reverse_proxy 127.0.0.1:3000` (normal managed certificate); (b) NEW catch-all HTTPS site block for customer domains → same reverse_proxy, certificates via **`on_demand_tls` with `ask` pointing at the local ask-endpoint** (global options block; exact syntax/Caddy version on the box — Architect verifies). Applied MANUALLY on deploy (live `/etc/caddy/Caddyfile` stays authoritative). Deploy notes: DNS `rate.albusto.com` A-record = owner action in GoDaddy (browser-only, no API — memory constraint).
- **FR-13 (ask-endpoint).** Unauthenticated local HTTP endpoint (mounted like the other public routers; minimal `src/server.js` mount edit — flagged, protected file). Contract: Caddy passes the candidate domain (query param); respond **200** iff the domain row is `verified`/`active` AND the owning company's `rate-me` installation is connected; **else 404**. Responses carry NO details (empty/uniform body both ways). Localhost caller → NOT per-IP throttled into breaking TLS handshakes, but protected by an in-memory decision cache (~60s) + hard cap (Architect). `rate.albusto.com` itself never depends on the ask path (it has its own site block).
- **FR-14 (observability).** Structured logs: domain set/verify attempts (`company_id`, domain, result, error), every ask decision (domain, allow/deny), every rating write (`company_id`, token id, stars, has_feedback, replay flag), sampled/counted public-404s. Marketplace audit events via existing `writeEvent`: `settings_updated` (scaffold) + NEW `domain_added` / `domain_verified` / `domain_activated` / `domain_removed` (shape — Architect).

### Нефункциональные требования

- **NFR-1 (token strength).** ≥128-bit entropy (`crypto.randomBytes(16+)` → base64url), unique-constraint retry on collision; format-guard regex before DB lookup. Explicitly do NOT copy the 64-bit estimate token.
- **NFR-2 (no enumeration).** Uniform 404 for malformed/unknown/expired/foreign-host/app-disconnected tokens — indistinguishable responses; per-IP `express-rate-limit` on public GET (~60/min) and POST (~10/min) (Architect tunes; publicAuth precedent).
- **NFR-3 (tenant isolation).** Every query company-scoped; the token row's `company_id` binds ALL subsequent reads (job/tech/profile/branding); Host-binding per FR-8; NO cross-tenant read path. Mandatory tests: isolation (foreign-host token → 404), replay idempotency, 401/403 on authed settings/domain endpoints, ask-endpoint silence.
...
- **A1 — page path + serving mechanism.** `/r/:token` vs `/:token`; host-gated main-SPA route (+ `PUBLIC_AUTH_PATHS` entry) vs separate minimal bundle; how NFR-5 is enforced for the chosen mechanism.
- **A2 — per-app settings dispatch shape.** Refactor seam for `validateRelySettingsInput`/`buildSettingsResponse` keeping rely suites byte-green; where rate-me validation lives.
- **A3 — domain endpoints addressing.** Inside `/api/marketplace` (app-key-scoped) vs a rate-me-specific router; response embedding of domain state in settings GET.
- **A4 — ask-endpoint path + cache + Caddy specifics.** Exact ask URL, in-memory cache TTL/size, `on_demand_tls` syntax for the Caddy version on the prod box, and the activation signal (FR-11).
- **A5 — CNAME check mechanics.** `dns.promises.resolveCname` primary (owner decision = CNAME-only); behavior behind CDN CNAME-flattening (resolved-A fallback allowed or documented limitation) + resolver timeout policy.
- **A6 — mint seam surface.** Service-only vs additionally a permission-gated authed endpoint for smoke tests; job/tech existence validation at mint time.

### Out of scope (explicit)

- SMS sending and ANY automatic trigger (job-Done etc.) — nothing mints tokens automatically this phase.
...
- Configurable happy threshold [OWNER: 5★ only]; per-technician Google links.
...
- `backend/src/routes/marketplace.js` + `backend/src/services/marketplaceService.js` — whitelist + per-app settings dispatch (FR-9), domain endpoints (FR-10) if placed here.
- NEW `backend/src/routes/public-rate.js` (name per Architect) — public GET/POST + ask-endpoint; NEW `backend/src/services/rateMeService.js` + `backend/src/db/rateMeQueries.js`; migration `backend/db/migrations/172_*.sql`.
...
- `src/server.js` — mount-only lines (flagged, NFR-10).
...
- `infra/Caddyfile` + `infra/README.md` — new blocks + on_demand_tls + apply procedure (FR-12).

### Затронутые интеграции

- **Twilio / Front / Zenbooker / Gmail / Stripe / VAPI — none.** (Technician ids originate from Zenbooker but are read from existing CRM tables/JSONB; no ZB API calls.)
- **Google — redirect-only** (outbound link to the company's Google-review URL; no API).
- **DNS/registrar (GoDaddy)** — owner-side manual records (`rate.albusto.com` A-record; tenant CNAMEs are the tenants' own registrars).
- **Caddy / Let's Encrypt** — NEW on-demand certificate issuance for customer domains (ask-endpoint gated).
- **S3/objects storage** — presigned company-logo reads (existing mechanism).

### Защищённые части кода (НЕЛЬЗЯ ломать)

- `src/server.js`, `frontend/src/lib/authedFetch.ts`, `frontend/src/hooks/useRealtimeEvents.ts` (protected list) — server.js gets ONLY the flagged mount lines.
- Existing public routers (`public-estimates.js`, `public-invoices.js`, `publicAuth.js`) and their `/api/public` behavior — byte-identical.
...
- Marketplace install/disconnect/credential flows and `marketplace_events` writes — extend, don't modify.
- `technician_profiles` / `technician_base_locations` / `jobs.assigned_techs` shapes — read-only.
- `companies` table — read-only here (no new columns; the Google link lives in installation metadata).
- Existing `infra/Caddyfile` blocks (albusto.com / app+api / auth) — append-only.
- FSM subsystem, leads/jobs flows, Pulse — untouched.

---

## RATE-ME-CRM-002 — humane, conversion-focused Rate Me page personalized from the job + review→job attribution + rebooking screens + "Send rating link" from the Job card (Phase 2, UX/data — ADDITIVE on the deployed 001) (2026-07-14)

> Status: requirements (Product 01). **Phase 2 of RATE-ME-CRM-001.** The infrastructure — opaque ≥128-bit tokens, the public token API, the `/r/:token` page, the host-gate, the `rate-me` marketplace app + `google_review_url` setting, and migration 177 (`rate_tokens` / `technician_ratings` / `rate_me_domains`, with `rate_tokens.job_id` already persisted) — is **already DEPLOYED**. This phase is purely ADDITIVE UX + data: it (a) makes the public page humane and **personalized from the job**, (b) rewrites `RatePage.tsx` into a **7-screen flow**, (c) turns 5★ into a **new-tab Google review with a click beacon**, (d) adds **token-level attribution** (opened / clicked / sent) surfaced ON the Job card, (e) adds a dispatcher **"Send rating link"** (SMS / Email / Copy) action, and (f) adds a per-company **`booking_url`** rate-me setting for rebooking. **Do NOT break 001** (isolation, uniform-404, replay-idempotency, `google_review_url`, rely-leads settings). Binding owner decisions are marked **[OWNER]** and were locked in the RM2 interview. Notable locks: palette = **Albusto v2 / violet `#7F42E1`** (NOT the warm "Blanc" paper; the word "Blanc" NEVER renders in UI); rating stars = **classic gold** (rating-semantics exception to the one-accent rule); threshold = **exactly 5★, NOT configurable**; **5★ opens Google in a NEW TAB** (`window.open` + beacon, never `location.replace`); **chips are thought-prompts that insert NOTHING into the textarea**; **PII minimal — only the customer FIRST name** leaves the public context.

### Verified code/schema facts (downstream agents: do NOT re-derive, do NOT contradict)

- **Personalization source.** `backend/db/migrations/031_create_jobs.sql`: `service_name TEXT`, `start_date TIMESTAMPTZ`, `customer_name TEXT` (full only), `contact_id BIGINT → contacts(id)`, `assigned_techs JSONB`. `contacts.first_name` exists (mig 025). Resolve first name via `jobs.contact_id → contacts.first_name`; fallback = first whitespace token of `jobs.customer_name`. `jobsService.rowToJob` exposes those fields; `getJobById` does NOT currently join contacts.
...
- **Company contacts + booking_url.** `companies` already carries customer-facing `contact_phone` / `contact_email` (+ `name` / `logo_storage_key` used by `getPublicContext`). There is **NO `booking_url` column anywhere.** DECISION: `booking_url` = a **rate-me marketplace setting** (JSONB in `marketplace_installations.metadata.settings`, exactly like `google_review_url`) — **no DB migration for it.**
- **Settings whitelist.** `marketplaceService.js`: `SETTINGS_ENABLED_APP_KEYS = Set(['rely-leads','rate-me'])`; `validateRateMeSettingsInput` validates `google_review_url` and RETURNS `{google_review_url}`; `buildRateMeSettingsResponse` → `settings:{google_review_url}`; `updateAppSettings` REPLACES the whole settings blob (`marketplaceQueries.setInstallationSettings`) — so any new key MUST be parsed AND returned by the validator or PUT wipes it.
- **Send-link seams.** SMS: `conversationsService.sendMessage(conversationId,{body,author:'agent'})` + `getOrCreateConversation(customerE164, proxyE164, companyId)` + `resolveCompanyProxyE164(companyId)` (`messagingHelper.js`); exact reusable, wallet-gated pattern = `backend/src/routes/jobs.js` L831-867 (On-the-way notify, `WALLET_BLOCKED`/`SMS_FAILED`). Email: `emailService.sendEmail(companyId,{to,subject,body,textBody,…})` (throws if mailbox disconnected). Copy: return the `/r/:token` URL from mint; FE copies to clipboard.
- **Mint + public shapes (extend, don't fork).** `rateMeService.mintToken(companyId,{jobId,techId,techName}) → {token,url}`; `getPublicContext(token,hostCompanyId)`; `submitRating(token,{stars,feedback},hostCompanyId)`; `googleReviewUrl(metadata)`. `backend/src/routes/public-rate.js`: `RATE_TOKEN_RE=/^[A-Za-z0-9_-]{22,64}$/`, GET `/rate/:token` (60/min), POST `/rate/:token/rating` (10/min). `POST /apps/rate-me/tokens → mintToken` in `backend/src/routes/marketplace.js`. `middleware/rateHostGate.js` allowlist already covers `/api/public/rate/…` (beacon needs no new prefix; `/r/` stays).
- **Job-card home.** `frontend/src/components/jobs/JobDetailPanel.tsx` → left column `JobOpsSection` (in `JobStatusTags.tsx`, `JOB-ACTIONS-SLIM-001` band) is the home for the Rate Me status block + Send-link action. Modal precedent = `OnTheWayModal.tsx`; data via `useJobDetail.ts`.
- **Migration numbering.** Next free = **178** (highest is 177_rate_me). Parallel sessions may claim it — RE-CHECK vs origin/master at push and renumber both ends if taken. New migration is additive/idempotent, NOT boot-registered (applied via psql at deploy, same as 177).
...
- **US-RM2-1 (customer, happy path).** Sarah opens the tokenized link right after Alex leaves → a mobile page greeting "Hi Sarah," with "How did Alex do?" and "Refrigerator repair · Friday, Jul 12", five gold stars → taps **5★** → rating records → a Google-helper screen ("Wonderful — thank you", prompt chips, violet "Write my Google review") → the button fires a click beacon and opens the company's Google review in a **new tab**; behind it our happy thank-you "You're the best, Sarah." stays visible with a quiet "Book your next visit" link + contacts.
- **US-RM2-2 (customer, recovery path).** Sarah taps **3★** → no auto-post; a private feedback screen ("Thanks for being straight with us", textarea + gentle topic chips + "Private — only ABC Homes sees this") → "Send to the team" posts stars+feedback → "Thank you — we hear you. A manager will reach out" with talk-now contacts and NO rebooking button.
- **US-RM2-3 (customer, already rated / expired).** Re-opening a rated link → "You've already rated this visit." + a warm rebooking block (violet **Book Visit** + contacts). An EXPIRED-but-recognized link → a soft "This link has expired." with the same rebooking block. A genuinely unknown/foreign token → the plain 001 "link no longer available" (no company data).
- **US-RM2-4 (dispatcher, send a link from the job).** A dispatcher opens a job's card, sees the "Rate Me" block, taps **Send rating link**, picks **SMS**, **Email**, or **Copy link** → the token is minted for (company, job, technician) and delivered; `sent_at`/`sent_via` stamp; the block then shows "Rating link sent · SMS to (617)…".
- **US-RM2-5 (dispatcher, watch attribution).** After the customer acts, the same Job-card block fills in the timeline: **sent → opened → rated (★N) → opened Google review**, every step tied to the exact `job_id` — "we know exactly which job the review came from."
- **US-RM2-6 (owner, configure rebooking).** The owner opens Rate Me app settings and sets **Booking URL** (`https://…`) alongside the Google review link → the customer screens' "Book Visit" / "Book your next visit" now point at it; leaving it blank simply hides those affordances (no dead buttons).
- **US-RM2-7 (isolation probe).** A dispatcher of company A cannot send a link on, or read the rate status of, company B's job (404/empty). A token/beacon for company B opened on company A's host → uniform 404. The public context never leaks more than the first name.

### Функциональные требования

- **FR-RM2-01 (personalization from the job — first name / service / date, PII-minimal) [OWNER].** Extend the public GET context to resolve, from the token's `job_id`: the customer FIRST name (`jobs.contact_id → contacts.first_name`; fallback = first whitespace token of `jobs.customer_name`; if neither → omit the name, greet generically "Hi there,"), the service label (`jobs.service_name`), and the visit date (`jobs.start_date`, formatted in the COMPANY timezone, e.g. "Friday, Jul 12"). The page greets "Hi {first}," and shows "{service} · {date}" under "How did {tech} do?". **PII MINIMAL — ONLY the first name may leave the public context; never last name, never the customer's phone/email, never ids, never another company's data** [OWNER]. Missing `service_name`/`start_date` degrade gracefully (omit the missing part; never render "—"). Additive to 001's fields. **Guard: SAB-CONTEXT-PII-LEAK.**
- **FR-RM2-02 (per-company branding + contacts + booking-URL — never hardcoded).** Branding (company name + logo), contact `tel:`/`mailto:` (`companies.contact_phone` / `contact_email`), and the rebooking `booking_url` are ALL per-company, resolved server-side from the token's company; NOTHING is hardcoded (no "ABC Homes", no mockup phone). `booking_url` comes from the rate-me setting (FR-RM2-19); phone/email from `companies`. Any value not configured is omitted on the page (no empty rows, no dead buttons) — mirrors 001's Google-link-missing fallback.
- **FR-RM2-03 (Screen 1 — personalized invitation).** Company logo (round, only if `company_logo_url`; `onError` hides the img — 001 NFR-8) + company-name eyebrow; greeting "Hi {first},"; headline "How did {technician_name || 'our technician'} do?"; subline "{service} · {date}"; one row of **five gold stars** (empty = neutral gray) with ≥44px hit targets; hint "Tap a star to rate". Single focus: the stars. No chrome, no "Blanc".
- **FR-RM2-04 (Screen 2 — 5★ Google-review helper).** On 5★ the rating records FIRST (001 POST contract), then this screen shows a small gold-star row + "Wonderful — thank you." + a humane ask ("A quick word on Google means a lot to a small local crew like ours. It takes about a minute.") + a prompt block "Not sure what to mention?" with chips **Punctuality · Clear explanation · Tidy work · Fair price · Friendliness** + fine print "Just prompts — your own words matter most." + violet primary **"Write my Google review"** (fires the click beacon then opens the company Google link in a NEW TAB — FR-RM2-10/15) + a quiet "Maybe another time" that drops to Screen 3. Shown ONLY when `google_review_url` is configured; otherwise 5★ goes straight to Screen 3 (001 fallback preserved).
- **FR-RM2-05 (Screen 3 — happy thank-you).** Centered gold-star mark + "You're the best, {first}." + a warm line + technician signature ("— {tech} & the {company} crew") + a **QUIET violet text link** "Book your next visit →" (to `booking_url`, NOT a filled button — tone) + contacts (`tel:`/`mailto:`). Happy terminal state, reached after the Google new-tab or "Maybe another time".
- **FR-RM2-06 (Screen 4 — 1–4★ private feedback).** On 1–4★ (NO auto-POST): a small gold-star row reflecting the pick (re-selectable until Send) + "Thanks for being straight with us." + "Tell us what missed the mark — this goes to our team, and won't be posted publicly." + a **textarea** ("What could we have done better?") + gentle topic chips **Timing · Communication · The repair · Pricing** + a privacy plaque "Private — only {company} sees this" + violet primary **"Send to the team"**. Submit posts rating+feedback (feedback optional/skippable) → Screen 5. **No Google, nothing public.**
- **FR-RM2-07 (Screen 5 — feedback thank-you).** Centered green check + "Thank you — we hear you." + "A manager from {company} will reach out to make this right." + "Prefer to talk now?" with `tel:`/`mailto:` contacts. **NO rebooking button** — selling a new visit to an unhappy customer is the wrong tone [OWNER]. Feedback terminal state.
- **FR-RM2-08 (Screen 6 — already-rated → rebooking).** When the token is already rated (GET `already_rated` OR POST replay `already_recorded` — 001 idempotency): violet check + "You've already rated this visit." + "Thanks again, {first} — it means a lot to {tech} and the team." + a rebooking block "Need help again? / Book your next service anytime" + violet **"Book Visit"** (→ `booking_url`) + contacts. No star picker. Turns a dead end into a warm rebooking lead.
- **FR-RM2-09 (Screen 7 — expired → rebooking; truly-invalid stays generic).** An EXPIRED but recognized token (valid token past `expires_at`) → a soft screen (NOT a raw 404): clock mark + "This link has expired." + "Rating links stay active for a while after your visit." + the SAME rebooking block as Screen 6 (violet "Book Visit" + contacts), using THAT token's company. **BUT unknown / malformed / foreign-host / app-disconnected tokens KEEP 001's uniform generic "This link is no longer available." with NO company branding, booking, or contacts** — revealing company data for a token we don't own would break 001's anti-enumeration + host-isolation contract. The rebooking block is shown for EXPIRED (company known) only. (See OPEN QUESTIONS FOR OWNER.)
- **FR-RM2-10 (threshold logic — 5★→Google new-tab, 1–4★→feedback) [OWNER].** Threshold is `stars === 5` ONLY, NOT configurable. 5★ → record, then open the company `google_review_url` in a **NEW browser tab** via `window.open` AFTER firing the click beacon (FR-RM2-15) — **never `location.replace`/same-tab**, because our thank-you (Screen 3) must stay visible behind the new tab. 1–4★ → private feedback (Screen 4) → feedback thank-you (Screen 5). If `google_review_url` is unset, 5★ still records and lands on Screen 3 (no dead end). **Guard: SAB-GOOGLE-SAME-TAB.**
- **FR-RM2-11 (chips are prompts, not text generators) [OWNER].** On BOTH the 5★ helper (Screen 2) and the feedback screen (Screen 4), the chips/bubbles are **thought-direction prompts ONLY**. Tapping a chip inserts NOTHING into the textarea and generates no review/feedback text (pre-writing the customer's words reads as coercion). The 5★ chips carry the fine print "Just prompts — your own words matter most." **Guard: SAB-BUBBLE-INSERTS-TEXT** (RED if a chip mutates the textarea value).
- **FR-RM2-12 (rebooking + contacts — placement rules) [OWNER].** Affordance intensity is tone-matched: **filled violet "Book Visit"** on already-rated (Screen 6) + expired (Screen 7); a **quiet violet text link** "Book your next visit →" on the happy thank-you (Screen 3); **NO rebooking** on the feedback thank-you (Screen 5). Contacts (`tel:` company phone + `mailto:` company email) appear on Screens 3, 5, 6, 7. Screens 1, 2, 4 carry no contacts/rebooking (focus). Every booking/contact value is per-company and omitted when unconfigured.
- **FR-RM2-13 (gold stars, violet single-action, Albusto v2) [OWNER].** Rating stars = **classic gold** (filled ≈ `#E0A72C`, empty neutral gray ≈ `#D2D2D0`) — the sole exception to the one-accent rule (gold = rating semantics). Every OTHER action (primary buttons + text links: "Write my Google review", "Send to the team", "Book Visit", "Book your next visit") uses the single violet accent `--blanc-accent #7F42E1` (`--blanc-accent-soft #E7DBFD` for soft marks/plaques). Palette strictly **Albusto v2** (`frontend/src/styles/design-system.css`, `--blanc-*`: neutral `#F1F1F0` canvas, white surfaces, ink `#191919/#6E6E6E/#8A8A8A`, hairline `rgba(25,25,25,.08)`, Manrope headings + IBM Plex Sans body, radii 10/16/22/28). **The string "Blanc" NEVER renders in UI.**
- **FR-RM2-14 (attribution schema — Migration 179, additive) [OWNER].** Migration **178** (re-check the free number vs origin/master at push; renumber if taken) ALTERs `rate_tokens` ADD (all nullable): `opened_at TIMESTAMPTZ`, `google_click_at TIMESTAMPTZ`, `sent_at TIMESTAMPTZ`, `sent_via TEXT`. **No `booking_url` column** (that is a JSONB setting — FR-RM2-19). Additive, idempotent, rollback-safe; NOT boot-registered (applied via psql at deploy, same as 177). `sent_at`/`sent_via` are single-valued → **most-recent-send wins** (matches the single "Rating link sent · {via}" step on the Job card).
- **FR-RM2-15 (event stamping — opened_at on GET, click beacon POST).** `opened_at` is stamped on the FIRST public GET context for the token (do NOT overwrite on later GETs). Add a public, rate-limited, host-bound, **idempotent** `POST /api/public/rate/:token/click` beacon that stamps `google_click_at` (first-click wins) and returns a minimal 200; the client fires it JUST BEFORE `window.open` on 5★. The beacon derives company/job from the token ONLY (never the body), is covered by the existing `rateHostGate` allowlist (no new public prefix), and — like every 001 public route — returns uniform 404 for unknown/foreign/malformed tokens. **Guards: SAB-GOOGLE-SAME-TAB, SAB-ATTRIBUTION-WRONG-JOB.**
- **FR-RM2-16 (Job-card "Rate Me" status block — attribution surfaced on the job).** On the Job detail panel (left column `JobOpsSection`, JOB-ACTIONS-SLIM band) a "Rate Me" block renders the token lifecycle for THIS job as a timeline: **Rating link sent** ({sent_at} · {sent_via}) → **Opened** ({opened_at}) → **Rated** (★N + rating created_at) → **Opened Google review** ({google_click_at}) — each step present only when its timestamp exists. Every event and the rating are tied to `rate_tokens.job_id` (schema 177). If no token exists for the job yet, the block shows only the "Send rating link" action (FR-RM2-18). **Guard: SAB-ATTRIBUTION-WRONG-JOB.**
- **FR-RM2-17 (job-rate-status GET by job_id — authenticated, tenant-scoped).** A NEW authenticated endpoint returns the rate status for a given `job_id` (token existence, `sent_at`/`sent_via`, `opened_at`, rating stars + created_at, `google_click_at`) to feed FR-RM2-16. Company-scoped by `req.companyFilter.company_id` (NEVER `req.companyId`); a job of another company → 404/empty. Mounted under the authenticated marketplace/jobs surface; exact permission gate = Architect (jobs-scoped vs `tenant.integrations.manage`). **Guard: SAB-SENDLINK-CROSS-TENANT.**
- **FR-RM2-18 (Send rating link from the Job card — SMS / Email / Copy) [OWNER].** An authenticated dispatcher action on the Job card mints (or reuses/extends) a token for (company, job, technician) via the existing `rateMeService.mintToken` / `POST /apps/rate-me/tokens` seam and delivers the `/r/:token` link three ways: **SMS** (the `conversationsService`/`messagingHelper` seam — wallet-gated, the `jobs.js` On-the-way pattern), **Email** (`emailService.sendEmail` — errors if the mailbox is disconnected), or **Copy link** (returns the URL; FE copies to clipboard). On send, stamp `sent_at`/`sent_via`. SMS requires a customer phone and Email a customer email (from the job's contact); when a channel's recipient is missing, that channel is disabled with an honest reason (no silent failure). Everything company-scoped; sending on another company's job is impossible. **Guard: SAB-SENDLINK-CROSS-TENANT.**
- **FR-RM2-19 (booking_url — a rate-me marketplace setting) [OWNER].** Add `booking_url` to the rate-me app settings stored in `marketplace_installations.metadata.settings` (JSONB, alongside `google_review_url` — NO DB column). Extend `validateRateMeSettingsInput` to ALSO parse and **RETURN** `booking_url` (nullable; else string, `https:` scheme, ≤500 chars) and surface it in `buildRateMeSettingsResponse.settings`; add a `bookingUrl(metadata)` reader in `rateMeService` mirroring `googleReviewUrl`. **CRITICAL: because PUT replaces `metadata.settings` wholesale, the validator MUST keep returning `google_review_url` too — dropping it wipes the existing Google link.** Company phone/email stay in `companies` (already tenant-scoped), not in this setting.
...
- **NFR-RM2-3 (public route hardening).** GET context, POST rating, and POST click beacon remain unauthenticated but rate-limited per-IP (001 limiter precedent), token-format-guarded before any DB read, host-bound (foreign token/host → uniform 404), and derive company/job/tech from the token ONLY. The beacon is idempotent (first-click wins). No enumeration surface added.
- **NFR-RM2-4 (tenant isolation on authenticated surfaces).** Send-link and job-rate-status endpoints are company-scoped via `req.companyFilter.company_id` (NEVER `req.companyId`); operating on another company's job/token → 404/empty. **SAB-SENDLINK-CROSS-TENANT** = RED if either works on a foreign job.
...
- **NFR-RM2-6 (5★ new-tab).** 5★ uses `window.open` + a preceding beacon, never `location.replace`/same-tab. **SAB-GOOGLE-SAME-TAB** = RED if same-tab.
...
- **NFR-RM2-9 (fail behavior).** Logo presign failure → name-only render (001 precedent); beacon failure must NOT block the Google new-tab; SMS is wallet-gated (`WALLET_BLOCKED`/`SMS_FAILED` honest errors); Email errors if the mailbox is disconnected (honest error, no crash); unconfigured booking/contacts/Google → the affordance is simply omitted (no dead links).
- **NFR-RM2-10 (settings integrity).** `booking_url` validation (nullable / `https:` / ≤500) and the replace-on-PUT invariant that `google_review_url` survives; rely-leads settings GET/PUT stay byte-identical.
...
### Именованные sabotage-контроли (обязательны — каждый RED при снятии guard, green при возврате)

- **SAB-BUBBLE-INSERTS-TEXT** — a prompt chip mutates the feedback/review textarea (FR-RM2-11).
- **SAB-CONTEXT-PII-LEAK** — the public context returns extra PII or another company's data (FR-RM2-01 / NFR-RM2-2).
- **SAB-GOOGLE-SAME-TAB** — 5★ uses `location.replace`/same-tab instead of new-tab + beacon (FR-RM2-10/15).
- **SAB-SENDLINK-CROSS-TENANT** — send-link or job-rate-status works on a job of another company (FR-RM2-17/18).
- **SAB-ATTRIBUTION-WRONG-JOB** — an event or rating attaches to the wrong `job_id` (FR-RM2-15/16).

### Open items for the Architect

- **AR1 — send-link gate + mount.** Exact permission for the dispatcher send (jobs-scoped vs `tenant.integrations.manage`) and where the send-link + job-rate-status endpoints mount (inside `/api/marketplace` vs a jobs-scoped route).
- **AR2 — beacon transport + sequencing.** `fetch(..., {keepalive:true})` vs `navigator.sendBeacon`, and how `window.open` is sequenced after it so a slow/failed beacon never blocks the new tab (NFR-RM2-9). Popup-blocker note: `window.open` must fire in the click handler.
- **AR3 — token reuse vs re-mint** on a repeat "Send rating link" for the same job, and how `sent_at`/`sent_via` update (most-recent-send wins per FR-RM2-14).
- **AR4 — getPublicContext query + timezone.** The `LEFT JOIN jobs + contacts` shape and the source of the company timezone used to format `start_date`.
- **AR5 — RatePage rewrite structure.** Single page-state machine vs sub-views, preserving the `/r/:token` route + `PUBLIC_AUTH_PATHS` and importing ZERO CRM chrome (the PublicInvoicePayPage / 001 precedent).

### OPEN QUESTIONS FOR OWNER

- **Screen 7 "expired vs invalid" scope.** The mockup lumps "expired/invalid" into one rebooking screen showing the company's Book Visit + contacts, but 001's anti-enumeration + host-isolation contract forbids showing ANY company data for a token we don't own (unknown / malformed / foreign-host). This spec resolves it as: **EXPIRED (recognized token, company known) → humane rebooking screen with THAT company's Book Visit + contacts; UNKNOWN / MALFORMED / FOREIGN-HOST → the existing generic "link no longer available" with NO company branding/booking/contacts.** Please confirm this split is acceptable (i.e. truly-invalid links do NOT show a rebooking block).

### Out of scope (explicit)

- **Auto-send on job→Done** (and any automatic trigger) — later phase; nothing mints/sends automatically this phase. Send is a MANUAL dispatcher action.
- Referrals, rewards, coupons.
- Two-axis / multi-dimensional rating — single 1–5 star only.
- ABC-Homes Vercel switch or any change to their external site.
- Ratings analytics / dashboard / reporting UI — the Job-card block shows THIS job's lifecycle only, not aggregate analytics.
- Configurable happy threshold (stays 5★) [OWNER]; per-technician Google/booking links.
...
- **Frontend:** `frontend/src/pages/RatePage.tsx` (rewrite → 7-screen flow), `frontend/src/components/jobs/JobDetailPanel.tsx` + `JobStatusTags.tsx` (Rate Me block + Send-link action), a NEW Send-link modal (`OnTheWayModal.tsx` precedent), `frontend/src/hooks/useJobDetail.ts` + jobs/marketplace api clients, the Rate Me settings dialog (`booking_url` field).
...
- **Twilio — YES (send-link SMS only)** — the existing conversations/messaging seam, wallet-gated; no new Twilio surface.
- **Gmail / Email — YES (send-link email only)** — existing `emailService`.
- **Google — redirect-only** (new-tab to the company's Google-review URL; no API).
- **Zenbooker / Front / Stripe / VAPI — none** (technician/job data read from existing CRM tables/JSONB; no ZB writes).
- **S3/object storage** — presigned company-logo reads (existing mechanism).

### Защищённые части кода (НЕЛЬЗЯ ломать)

- `src/server.js`, `frontend/src/lib/authedFetch.ts`, `frontend/src/hooks/useRealtimeEvents.ts` — untouched (server.js only a minimal flagged mount if a new public route is truly unavoidable, +N/-0).
- All 001 contracts: public GET context + POST rating shapes (extend ADDITIVELY, keep existing fields), the uniform-404 quintet, replay idempotency (`rate_token_id UNIQUE`), the host-gate, the `google_review_url` setting, and rely-leads settings GET/PUT — byte-identical behavior; migration-177 tables read/extended only.
- `companies` — read-only here (no new columns; `booking_url` lives in installation metadata).
...
## INSPECTOR-AGENT-001 — агент «Инспектор»: ежедневный поиск зависших работ и лидов (2026-07-20)

**Как владелец**, я хочу marketplace-агента «Инспектор», который после включения каждый день (после 12:00 по таймзоне компании) просматривает работы и лиды, «застрявшие» в нефинальных статусах, и ставит диспетчеру задачу на те, что требуют внимания — чтобы ничего не зависало молча. Судит LLM, без жёсткого and/or-конструктора.

**Решения владельца (развилки → выбор):**
- Логика — LLM, НЕ конструктор условий. Гибкость — в свободной инструкции-промпте в настройках.
- Настройки (per-company, только tenant_admin): вкл/выкл; игнор-статусы работ (дефолт Visit completed, Job is Done, Canceled); игнор-статусы лидов (дефолт Converted, Lost); свободная инструкция с сильным дефолтным текстом. Списки статусов — из FSM компании.
- Кандидаты: работа с датой визита в прошлом + статус не из игнора; лид без активности со вчера (прокси `updated_at`, LLM видит реальные даты) + не из игнора.
- Повтор — ежедневный; дедуп через СУЩЕСТВУЮЩИЙ снуз задачи: открытая (в т.ч. снузнутая = сдвиг `due_at`) задача Инспектора подавляет новую. Снуз задачи — способ диспетчера заглушить; отдельного поля на работе/лиде НЕТ (Q3).
- Заметки — центр решения + финансовый срез (estimated/invoiced/due/paid): агент ловит «работа сделана, но продажи/оплаты нет» (job 1376) и «в прошлом + без оплаты неделю» (job 1345). ETA в заметке → пауза; без ETA → задача «проверить доставку».
- Агент ТОЛЬКО ставит задачи: не звонит, не меняет статус, не собирает оплату.
