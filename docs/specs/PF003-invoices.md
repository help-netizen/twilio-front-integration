...
- `Job` detail
- approved `Estimate`
- `Contact` или `Pulse` только как assisted entry point с обязательным выбором существующего `Job` или approved `Estimate` до сохранения
...
- standalone invoices не допускаются: документ должен быть связан как минимум с `job_id` или `estimate_id`, а `lead_id` может сохраняться как sales trace.
...
- Invoice можно создать из `Job` или approved `Estimate`; `Contact`/`Pulse` допустимы только как assisted entry point с обязательным выбором связанного `Job` или `Estimate`.
