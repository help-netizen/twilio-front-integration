...
⚠ **Отправка невозможна и в SETUP.** `bin/sanitize-env.sh` на staging обнуляет все
TWILIO_* и ротирует `EMAIL_TOKEN_ENCRYPTION_KEY` — так и задумано: прогон теста не
должен написать живому клиенту. Следствие: **нельзя привести инвойс в issued** — его
`status` принадлежит воркфлоу (`WORKFLOW_CONTROLLED_UPDATE_FIELDS`), и единственный
путь туда — реальная отправка. У эстимейтов такой проблемы нет: `sent` ставится
обычным PUT.

Поэтому кейсы, которым нужен **выставленный** инвойс, помечены
`test.skip(!CAN_DISPATCH_DOCUMENTS)` (`E2E_CAN_DISPATCH_DOCUMENTS=1` там, где отправка
безопасна) — честный пропуск вместо красного, который никогда не позеленеет:
- `invoice-detail-redesign.spec.ts` — «issued actions + status-safe destruction» (P0),
  «collect-payment prefills balance» (P1).

**Дыра в покрытии, а не закрытый вопрос.** Закрыть её можно только продуктовым
решением — действием «отметить как отправленный / отдал клиенту на месте», которое
переводит инвойс в issued без сообщения. Это решение владельца; до него состояние
issued покрыто только на уровне юнитов.
