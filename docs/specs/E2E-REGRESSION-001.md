...
- **Креды — только через env** (`E2E_ADMIN_USER`/`E2E_ADMIN_PASS`, `E2E_PROVIDER_*`), которые ставит владелец (локально/CI). В коде/репо паролей нет; Claude их не вводит и не хранит.
