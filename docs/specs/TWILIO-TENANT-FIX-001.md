...
1. locks both target tables and the binding table, and fails if either target
   has NULL company scope or a subaccount SID has ambiguous bindings;
...
5. deterministically retains the most complete transcript and most progressed
   inbox row for any existing duplicate tenant-pair;
6. adds `UNIQUE(company_id, transcription_sid)` and
   `UNIQUE(company_id, event_key)` with guarded, re-apply-safe names.
...
- `T-blast`: master and subaccount share CallSid, transcription SID, and event
  ...
  byte-unchanged.
- Media: missing/forged/expired token, call mismatch, account mismatch, and
  cross-company token binding are rejected before session creation/audio.
